...
/*use "D:\DDI\Ghana\10%_2021PHC_STATA\stata\mortality_10%_20221011d.dta",clear
egen child_died=max( mr02c <18),by( nqid )
duplicates drop nqid, force
keep nqid child_died
merge 1:1 nqid using "D:\DDI\Ghana\10%_2021PHC_STATA\stata\housingandsanitation_10%_20221011d.dta",gen(_merge_child_death)

/*    Result                      Number of obs
    -----------------------------------------
    Not matched                       994,707
        from master                         0  (_merge==1)
        from using                    994,707  (_merge==2)

    Matched                            12,375  (_merge==3)
    -----------------------------------------*/


replace child_died=0 if child_died==.8*/

use "D:\DDI\Ghana\10%_2021PHC_STATA\stata\housingandsanitation_10%_20221011d.dta" ,clear

merge 1:m nqid using "D:\DDI\Ghana\10%_2021PHC_STATA\stata\defactopopn_10%_20221011d.dta",gen(_merge_hh)


/*    Result                      Number of obs
    -----------------------------------------
    Not matched                       164,960
        from master                   164,960  (_merge_hh==1)
        from using                          0  (_merge_hh==2)

    Matched                         3,083,572  (_merge_hh==3)
    -----------------------------------------*/
...
gen hh_weight=popweight

gen ind_weight=weight
*Individual weight was changed from popweight to weight in 2025"
...
gen education=0 if p12a==1
replace education=1 if inrange(p12b, 1,2)
replace education=2 if inrange(p12b, 3, 16)
replace education=3 if inrange(p12b, 6, 16)
...
gen alone=(usual_members==1)

gen death_hh=(deaths>=1)
replace death_hh=0 if deaths==.
