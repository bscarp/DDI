...
rename  edad age
*Marital status
destring SITUA_CONYUGAL,replace
gen married=inlist(SITUA_CONYUGAL,5,6,7)
replace married=. if SITUA_CONYUGAL==.

gen married2=inlist(SITUA_CONYUGAL,1,5,6,7)
replace married2=. if SITUA_CONYUGAL==.

*HH head
destring parentesco,replace
gen hh_head=(parentesco==101)
*hh house
clonevar hh_size= hhsize
*share of below 15
gen child_u15 = age < 15 if age < .
bysort hh_id: egen n_u15 = total(child_u15)
gen share_u15 = n_u15 / hhsize if hhsize > 0
*share of above 60
gen adult_a60 = age >= 60 if age < .
bysort hh_id: egen n_a60 = total(adult_a60)
gen share_a60 = n_a60 / hhsize if hhsize > 0
*female head
destring JEFE_SEXO,replace
gen hh_female_head = (JEFE_SEXO==3)
...
egen admin= concat(admin1 admin2), format(%25.0g)
destring admin, replace

**********Run do files - lables for admin2*********************

drop admin2
replace admin admin2
****
...
replace age_group=3 if age>=45 & age<65
...
*** Child Mortality
destring HIJOS_FALLECIDOS, replace
codebook HIJOS_FALLECIDOS, tab(30)
recode   HIJOS_FALLECIDOS (99=.)

gen mujer_12 = (female==1 & age>=12)
tab mujer_12, m

bys hh_id: egen hh_mujer12 = max(mujer_12)

gen  mortality=(HIJOS_FALLECIDOS>0) if mujer_12==1 & HIJOS_FALLECIDOS!=.
replace mortality=0 if HIJOS_NAC_VIVOS==0 & mujer_12==1
tab  mortality if mujer_12==1 , m 

egen child_mortality_hh=max(mortality), by(hh_id)
replace child_mortality_hh=0 if hh_mujer12==0 
...
replace ind_emp2=0 if sittra=="6"

** Youth Idle Rate 
destring asisten ,replace
recode   asisten (9=.)

gen school_new=(asisten==3)
replace school_new=. if (asisten==.)
...
gen     youth_idle2  = (ind_emp2==0 & asisten==3) if d_age_15_24==1
replace youth_idle2 =. if asisten ==. & ind_emp2==.
