...
*Note: The raw datasets were downloaded in .csv format by region from the web portal. All files were converted to .dta format and merged to create individual- and household-level datasets. After obtaining the household and individual-level data, the following merge was performed:

use "D:\DDI\Philippine\Philippine_HH_Sample.dta",clear
egen hh_id= concat( reg prv mun hsn ), format(%25.0g) punct(_)
duplicates tag hh_id, generate(dup)

tab dup

/*      dup |      Freq.     Percent        Cum.
------------+-----------------------------------
          0 |  5,183,405       98.46       98.46
          1 |     80,930        1.54      100.00
------------+-----------------------------------
      Total |  5,264,335      100.00
*/
drop dup
duplicates drop hh_id, force
*removing duplicates
save "D:\DDI\Philippine\Philippine_HH_Sample_rem_dup.dta",replace
use "D:\DDI\Philippine\Philippine_Ind_Sample.dta", clear
egen hh_id= concat( reg prv mun hsn ), format(%25.0g) punct(_)
merge m:1 hh_id using "D:\DDI\Philippine\Philippine_HH_Sample_rem_dup.dta"
   /*Result                           # of obs.
    -----------------------------------------
    not matched                             0
    matched                        21,372,548  (_merge==3)
    -----------------------------------------
*/
drop _merge
save "D:\DDI\Philippine\Philippine_Census_2020_Combined.dta",replace

egen ind_id= concat( reg prv mun hsn lna ), format(%25.0g) punct(_)
...
------------+-----------------------------------
          0 | 21,272,930       99.53       99.53
          1 |     99,618        0.47      100.00
------------+-----------------------------------
      Total | 21,372,548      100.00
...
save "D:\DDI\Philippine\Philippine_Census_2020_final.dta",replace
use "D:\DDI\Philippine\Philippine_Census_2020_final.dta",clear
destring p13a p13b p13c p13d p13e p13f p14_prvmun p14_recode p15_prvmun p15_recode p16 p17 p18 p19_prvmun p20 p19_recode p21 p22 p23 p24_prvmun p24_recode p25 p26 p27 p28 popwgt,replace

merge m:1 reg prv mun using "D:\DDI\Philippine\urban and rural.dta",gen(_merge_ur)


/*   Result                           # of obs.
    -----------------------------------------
    not matched                             0
    matched                        21,322,739  (_merge_ur==3)
    -----------------------------------------

*/
save "D:\DDI\Philippine\Philippine_Census_raw.dta",replace 
********Run the variable lables do file************************
gen age=p5 


drop if p5<15

gen country_name="Philippines"

gen country_abrev="PHL"

gen country_dataset_year="Philippines Census 2020"

*decode reg , gen(admin1)
clonevar admin1=reg
clonevar admin2=prv

gen hh_weight=hhwgt

gen ind_weight=popwgt

***Urban=1 or Rural=0***

gen urban_new=1 if urb==1
replace urban_new=0 if urb==2
...
****Disability****
clonevar seeing_diff_new=p13a
replace seeing_diff_new=. if p13a==9
clonevar hearing_diff_new=p13b
replace hearing_diff_new=. if p13b==9
clonevar mobility_diff_new=p13c
replace mobility_diff_new=. if p13c==9
clonevar cognitive_diff_new=p13d
replace cognitive_diff_new=. if p13d==9
clonevar selfcare_diff_new=p13e
replace selfcare_diff_new=. if p13e==9
clonevar comm_diff_new=p13f
replace comm_diff_new=. if p13f==9
...
gen everattended_new=(p17>0)
replace everattended_new=. if p17==999| p17==.

*Education - completed primary school
*This variable was created for computing multidimensional poverty
gen ind_atleastprimary_all=1 if inrange(p17 , 160, 899) 
replace ind_atleastprimary_all=0 if inrange(p17 , 0, 151)
replace ind_atleastprimary_all=. if p17==999

***Individual atleast Primary Education***

gen ind_atleastprimary=1 if inrange(p17 , 160, 899) & age>=25
replace ind_atleastprimary=0 if inrange(p17 , 0, 151)  & age>=25
replace ind_atleastprimary=. if p17==999 & age>=25

***Individual atleast secondary Education***

gen ind_atleastsecondary=1 if (inlist(p17, 250,202,203)|(inrange(p17 , 350, 899)))  & age>=25
replace ind_atleastsecondary=0 if (inrange(p17, 0,201)| inrange(p17 , 204, 249)|inrange(p17 , 251, 349)) & age>=25
replace ind_atleastsecondary= . if p17==999 & age>=25

gen school_new=(p18==1)
replace school_new=. if p18==.

gen education=0 if (p17==0)
replace education=1 if (inrange(p17, 1,201)| inrange(p17 , 204, 249)|inrange(p17 , 251, 349))
replace education=2 if (inlist(p17, 250,202,203)|(inrange(p17 , 350, 899)))

*Employment status

gen ind_emp=(p21<=9)
replace ind_emp=. if  p21==99

*Managerial

gen work_managerial=(p21==1)
replace work_managerial=. if female==0

*Manufacture

gen work_manufacturing=( p22==3)
replace work_manufacturing=. if ind_emp==. | p22==99 | p22==.

*Informal work

gen work_informal=inlist(p23, 1,4,5,6,7)
...
replace work_managerial2= 1 if ind_emp==1 & work_managerial==1 
...
gen ind_roof=inlist(b3, 1,2,3)
replace ind_roof=. if b3==. | b3==9

*walls

gen ind_wall=inlist(b4, 1)
replace ind_wall=. if b4==99 | b4==.

*floor

gen ind_floor=(b6==1)
replace ind_floor=. if b6==. | b6==9
**Living Condition
...
gen ind_toilet=inlist(h6, 1,2,3,4,5,6,7,9)
replace ind_toilet=. if h6==.

*electric

gen ind_electric=inlist(h7, 1,5)
replace ind_electric=. if h7==9 | h7==.

*cleanfuel

gen ind_cleanfuel=inlist(h8, 1,3)
replace ind_cleanfuel=. if h8==9|h8==.

*water

gen ind_water=inlist(h9, 1,2,3,4,5,6,8,10,12,13,14)
replace ind_water=. if h9==.

*Assets
*Refrig
gen ind_refrig=(h15a==1)
replace ind_refrig=. if h15a==9 | h15a==.
*radio
gen ind_radio=(h15g==1)
replace ind_radio=. if h15g==9 | h15g==.
*tv
gen ind_tv=(h15h==1)
replace ind_tv=. if h15h==9 | h15h==.
*telephone
gen ind_phone=(h15k==1)
replace ind_phone=. if h15k==9 | h15k==.
*computer
gen ind_computer=(h15n==1)
replace ind_computer=. if h15n==9 | h15n==.
*autos
gen ind_autos=(h15o==1)
replace ind_autos=. if h15o==9 | h15o==.
*motorcycle
gen ind_motorcycle=(h15p==1)
replace ind_motorcycle=. if h15p==9 | h15p==.
*bike
gen ind_bike=(h15q==1)
replace ind_bike=. if h15q==9 | h15q==.
*cell phone
gen cell_new=(h15l==1)
replace cell_new=. if h15l==9 | h15l==.
*Asset ownership
egen ind_asset_ownership=rowmean(ind_radio ind_tv ind_refrig ind_phone ind_autos ind_computer ind_bike ind_motorcycle)
...
*save "D:\DDI\Philippine\Philippine_Census_2020_Sample.dta",replace
...
*save "D:\DDI\Philippine\Philippine_Census_2020_Sample with missing.dta", replace
...
compress
save "D:\DDI\Philippine\Philippine_Census_2020_Sample_Cleaned_Individual_Data_Trimmed.dta", replace

duplicates drop hh_id, force

save "D:\DDI\Philippine\Philippine_Census_2020_Sample_Cleaned_Household_Level_Data_Trimmed.dta", replace
