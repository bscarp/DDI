...
use "D:\DDI\Tonga\Tonga_PHC2016_Mortality.dta",clear
gen death=rost_g20_reason==1 & rost_g20_dep_yrs==2016
egen death_hh=max(death), by(InterviewId)
fre death_hh
duplicates drop InterviewId, force
fre death_hh
save "D:\DDI\Tonga\Tonga_PHC2016_death.dta",replace
use "D:\DDI\Tonga\Tonga_PHC2016_Household.dta"
merge 1:1 InterviewId using "D:\DDI\Tonga\Tonga_PHC2016_death.dta",gen(_merge_death)
replace death_hh=0 if death_hh==.
merge 1:m InterviewId using "D:\DDI\Tonga\Tonga_ind.dta",gen(_merge_hh)
keep if _merge_hh==3

save "D:\DDI\Tonga\Tonga_raw.dta",replace
...
gen ind_emp2 = (d1_main_activity==1|d1_main_activity==2|d1_main_activity==3|d1_main_activity==4|d1_main_activity==5)
replace ind_emp2 = . if d1_main_activity==.
...
gen ind_water= cond(mi(g7a_drink_water),.,cond(inlist(g7a_drink_water,1,2,3,4),1,0))

*Sanitation
*rename toilet_new ind_toilet 
gen ind_toilet=cond(mi(g9a_toilet),., cond(g9a_toilet==1,1,0))
replace ind_toilet = 0 if g11_share_toilet ==1
*acces to electircity
*rename electric_new ind_electric 
gen ind_electric=cond(mi(g15a_lighting),.,cond(g15a_lighting==1|g15a_lighting==2,1,0))
*clean cooking fuel 
*rename fuelcook_new ind_cleanfuel 
gen ind_cleanfuel=cond(mi(g14a_cook_fuel),.,cond(inlist(g14a_cook_fuel ,1,2,4),1,0))
...
clonevar ind_tv=g17_hhld_goods__17
*bike
*rename bike_new ind_bike
clonevar ind_bike=g17_hhld_goods__6

*Motorcycle
*rename motorcycle_new ind_motorcycle
clonevar ind_motorcycle=g17_hhld_goods__4

*telephone
*rename phone_new ind_phone
gen ind_phone= g17_hhld_goods__20
clonevar cell_new=g17_hhld_goods__19
*refrig
*rename refrig_new ind_refrig
clonevar ind_refrig=g17_hhld_goods__9
*computer
*rename computer_new ind_computer
gen ind_computer=cond(mi(g17_hhld_goods__21) & mi(g17_hhld_goods__22),., cond(g17_hhld_goods__21==1|g17_hhld_goods__22==1,1,0))
