...
if ("IPUMS_Cleaned_Individual_Data_Trimmed.dta" %in% dta_list) {
  ...
  dck2 = dck2 %>%
    mutate(across(
      c(country_name, country_abrev, country_dataset_year, admin1, admin2),
      ~ as.character(as_factor(.x))
    ))
  dck2 = dck2 %>%
    mutate(
      country_dataset_year = sub(" IPUMS ", "_IPUMS_", country_dataset_year)
    )
  ...
  foreach(i = datasets) %do%
    {
      ...
      if (
        is.na(data_list$Subnational_1_feasible[data_list$File_Name == i]) &
          "admin1" %in% names(dck)
      ) {
        dck = dck %>% select(-admin1)
      }
      if (
        is.na(data_list$Subnational_2_feasible[data_list$File_Name == i]) &
          "admin2" %in% names(dck)
      ) {
...
if (
  "Final_Individual_DHS_only.dta" %in%
    dta_list |
    "Final_DHS_only.dta" %in% dta_list
) {
  plan(multisession, workers = 4)
  dta = dta_list[
    dta_list %in% c("Final_Individual_DHS_only.dta", "Final_DHS_only.dta")
  ][1]
  file_name = paste0(data_loc3, dta)
  dck2 = read_dta(file_name)
  dck2 = dck2 %>%
  ...
  dck2 = dck2 %>%
    mutate(
      country_dataset_year = sub(
        "_",
        "_DHS_",
        gsub("Cambodia2", "Cambodia", country_dataset_year)
      )
    )
  datasets = unique(dck2$country_dataset_year)
  foreach(
