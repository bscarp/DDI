...
wei_list = r_list[
  !r_list %in% sum_list &
    sub(".RData", "", r_list) %in%
      psu$Country_Survey_Date[
        !grepl("simple mean", psu$`Stata code FINAL November 2024`) &
          !grepl("svyset", psu$`Stata code FINAL November 2024`)
      ]
]
                  ...
                  across(
                    ...
                    list(
                      mean = ~ ifelse(
                        sum(!is.na(.x)) < 50,
                        NA,
                        wtd.mean(.x, ind_weight, na.rm = T) * 100
                      ),
                      mean_se = ~ ifelse(
                        sum(!is.na(.x)) < 50,
                        NA,
                  ...
                  across(
                    ...
                    list(
                      mean = ~ ifelse(
                        sum(!is.na(.x)) < 50,
                        NA,
                        wtd.mean(as.numeric(.x) - 1, ind_weight, na.rm = T) *
                          100
                      ),
                      mean_se = ~ ifelse(
                        sum(!is.na(.x)) < 50,
