...
time = time %>%
  add_row(., point = "Processing", val = Sys.time(), diff = val - time$val[1])
# write.table(tibble(x="x", time="time"), file = "~/progress.csv", sep = ",", col.names = FALSE, row.names = FALSE)

#Process full population datasets
source(
  "./Estimate scripts/DDI summary calculations updated weights full sample.R"
)

time = time %>%
  add_row(
    .,
    point = "Summaries full",
    val = Sys.time(),
    diff = val - time$val[2]
  )
...
psu = read_xlsx(
  paste0(cen_dir, "Downloads/Census/Sampling design table.xlsx"),
...
r_list2 = r_list[
  !r_list %in% sum_list &
    !grepl("DHS", r_list) &
    sub(".RData", "", r_list) %in%
      psu$Country_Survey_Date[grepl(
        "svyset",
        psu$`Stata code FINAL November 2024`
      )]
]
...
with_progress({
  p = progressor(along = seq(length(r_list2) * (5 * 54 + 2)))
  foreach(
    r_name = r_list2,
  ...
  ) %dofuture%
    {
      ...
      r_sum_name = paste0(
        data_loc2,
        sub("\\.RData", "\\_Summary.RData", r_name)
      )
      ...
      dck = dck %>%
        mutate(
          disability_sev1 = factor(
            disability_some + 2 * disability_atleast,
            levels = c(0, 1, 2),
            labels = c("no", "some", "atleast")
          ),
          disability_sev2 = factor(
            disability_some + 2 * disability_alot + 3 * disability_unable,
            levels = c(0, 1, 2, 3),
            labels = c("no_v", "some_v", "alot", "unable")
          ),
          disability_sev1_hh = factor(
            disability_some_hh + 2 * disability_atleast_hh,
            levels = c(0, 1, 2),
            labels = c("no", "some", "atleast")
          ),
          disability_sev2_hh = factor(
            disability_some_hh +
              2 * disability_alot_hh +
              3 * disability_unable_hh,
            levels = c(0, 1, 2, 3),
            labels = c("no_v", "some_v", "alot", "unable")
          ),
          disability_any = factor(
            disability_any,
            levels = c(0, 1),
            labels = c("no_a", "any")
          ),
          disability_some = factor(
            disability_some,
            levels = c(0, 1),
            labels = c("no_s", "some_n")
          ),
          disability_alot = factor(
            disability_alot,
            levels = c(0, 1),
            labels = c("no_t", "alot_n")
          ),
          disability_unable = factor(
            disability_unable,
            levels = c(0, 1),
          ...
          disability_atleast = factor(
            disability_atleast,
            levels = c(0, 1),
            labels = c("no_l", "atleast_n")
          ),
          disability_any_hh = factor(
            disability_any_hh,
            levels = c(0, 1),
            labels = c("no_a", "any")
          ),
          disability_some_hh = factor(
            disability_some_hh,
            levels = c(0, 1),
            labels = c("no_s", "some_n")
          ),
          disability_alot_hh = factor(
            disability_alot_hh,
            levels = c(0, 1),
            labels = c("no_t", "alot_n")
          ),
          disability_unable_hh = factor(
          ...
          age_group10 = cut(
            age,
            c(14, 24, 34, 44, 54, 64, Inf),
            c("15 to 24", "25 to 34", "35 to 44", "45 to 54", "55 to 64", "65+")
          ),
          male = factor(1 - female, labels = c("Female", "Male")),
          age_sex = interaction(age_group10, male, lex.order = T, sep = " "),
          as_weight = case_when(
            age_sex == "15 to 24 Female" ~ 0.107823219959552,
            age_sex == "15 to 24 Male" ~ 0.114985391312909,
            age_sex == "25 to 34 Female" ~ 0.104530062206990,
            age_sex == "25 to 34 Male" ~ 0.109379985244955,
            age_sex == "35 to 44 Female" ~ 0.090482564098174,
            age_sex == "35 to 44 Male" ~ 0.092693136884689,
            age_sex == "45 to 54 Female" ~ 0.077908667689967,
            age_sex == "45 to 54 Male" ~ 0.077798687417348,
            age_sex == "55 to 64 Female" ~ 0.059590620455815,
            age_sex == "55 to 64 Male" ~ 0.056425978108021,
            age_sex == "65+ Female" ~ 0.060324813942667,
            age_sex == "65+ Male" ~ 0.048056872678913,
            TRUE ~ NA
          )
        )
      dck = dck %>%
        mutate(across(
          any_of(c("admin1", "admin2", "admin3", "admin_alt")),
      ...
      if ("strata2" %in% names(dck)) {
        dck = dck %>%
          select(-any_of(c("sample_strata"))) %>%
          rename(sample_strata = strata2)
      }
      if ("informal2" %in% names(dck)) {
        dck = dck %>%
          select(-any_of(c("work_informal"))) %>%
          rename(work_informal = informal2)
      }
      if ("work_informal2" %in% names(dck)) {
        dck = dck %>%
          select(-any_of(c("work_informal"))) %>%
          rename(work_informal = work_informal2)
      }
      if ("work_managerial2" %in% names(dck)) {
        dck = dck %>%
          select(-any_of(c("work_managerial"))) %>%
          rename(work_managerial = work_managerial2)
      }

      if ("admin1" %in% names(dck)) {
        if (n_distinct(dck$admin1) < 2) {
          dck = dck %>% select(-admin1)
        } else {
          dck = dck %>% mutate(admin1 = str_to_title(admin1))
        }
      }
      if ("admin2" %in% names(dck)) {
      ...
      if ("admin3" %in% names(dck)) {
        if (n_distinct(dck$admin3) < 2) {
          dck = dck %>% select(-admin3)
        } else {
          dck = dck %>% mutate(admin3 = str_to_title(admin3))
        }
      }
      if ("admin_alt" %in% names(dck)) {
      ...
      hh_a = c("health_exp_hh", "child_died", "death_hh")
      dis_a = c(
        "disability_any",
        "disability_some",
        "disability_alot",
      ...
      cou_a = dck %>%
        select(any_of(c("admin1", "admin2", "admin3", "admin_alt"))) %>%
        names()
      psu_a = c(
        "hh_id",
        "ind_weight",
      ...
      dom_a = dck %>%
        select(any_of(c(
          ...
          "cognition_unable",
          "selfcare_unable",
          "communicating_unable"
        ))) %>%
        names()
      df_age_sex = dck %>%
        mutate(n = sum(ind_weight, na.rm = TRUE)) %>%
        summarise(n = first(as_weight) * first(n), .by = age_sex) %>%
        arrange(age_sex) %>%
        as.data.frame()
      ...
      dck = dck %>%
        group_by(hh_id) %>%
        mutate(hh_id = cur_group_id()) %>%
        ungroup()

      psu2 = psu %>% filter(Country_Survey_Date == dataset)

      if (grepl("ssu", psu2$`Stata code FINAL November 2024`)) {
        if (!"ssu" %in% names(dck)) {
        ...
        dck2a = dck2a %>%
          as_survey_design(
            ids = c(psu, ssu),
            weights = c(ind_weight, NULL),
            strata = c(sample_strata, NULL),
            nest = TRUE
          )
        dck2b = dck2b %>%
          as_survey_design(
            ids = c(psu, ssu),
            weights = c(hh_weight, NULL),
            strata = c(sample_strata, NULL),
            nest = TRUE
          )
        dck2a$fpc$pps = FALSE
        ...
        dck2a = dck %>%
          ...
          filter(!is.na(psu) & !is.na(ind_weight))
        dck2b = dck %>%
          select(
            all_of(cou_a),
            all_of(dis_hh_a),
          ...
          filter(!is.na(psu) & !is.na(hh_weight)) %>%
          filter(!duplicated(hh_id))
        dck2a = dck2a %>%
          as_survey_design(
            ids = psu,
        ...
        dck2a = dck %>%
          ...
          filter(!is.na(hh_id) & !is.na(ind_weight) & !is.na(sample_strata))
        dck2b = dck %>%
          select(
            all_of(cou_a),
            all_of(dis_hh_a),
          ...
          filter(!is.na(hh_id) & !is.na(hh_weight) & !is.na(sample_strata)) %>%
          filter(!duplicated(hh_id))
        ...
        grepl("sample_strata", psu2$`Stata code FINAL November 2024`)
      ) {
        psu_a = psu_a[!grepl("ssu|psu", psu_a)]
        dck2a = dck %>%
          select(
      ...
      tabs = foreach(admin_grp = c("admin0", cou_a)) %dofuture%
        {
          ...
          tab_m_nr1 = foreach(
            agg_grp = c("All", "female", "urban_new", "age_group"),
            .combine = "full_join"
          ) %:%
            foreach(
              dis_grp = c(
            ...
            ) %dofuture%
            {
              ...
              tab = dck2a %>%
                ...
                complete({{ agg }}, {{ dis }}, {{ admin }}) %>%
                arrange({{ agg }}, {{ dis }}, {{ admin }}) %>%
                pivot_wider(names_from = {{ dis }}, values_from = -c(1:3)) %>%
                mutate(
                  Agg = paste0(agg_grp, " = ", {{ agg }}),
                  admin = {{ admin_grp }},
                  level = as.character({{ admin }}),
                  .after = 2
                ) %>%
                select(c(-1, -2))
            }
          if (
            ...
            tab_m_nr2 = tab_m_nr2 %>%
              mutate(
                ...
                across(contains("_n_"), ~ as.integer(NA))
              )
            names(tab_m_nr2) = sub(
              "everattended_new_",
              "health_exp_hh_",
              sub(
                "ind_atleastprimary",
                "child_died",
                sub("ind_atleastsecondary", "death_hh", names(tab_m_nr2))
              )
            )
          }
          ...
          tab_P4_nr1 = foreach(
          ...
          ) %dofuture%
            {
              ...
              tab = dck2a %>%
                mutate(disability_any = as.numeric(disability_any) - 1) %>%
                group_by({{ admin }}) %>%
                filter({{ dom }} == 1, .preserve = TRUE) %>%
                summarise(
                  across(
                    any_of(ind_a),
                    list(
          ...
          if (
            ...
            tab_P4_nr2 = foreach(
            ...
            ) %dofuture%
              {
                ...
                tab = dck2b %>%
                  mutate(
                    disability_any_hh = as.numeric(disability_any_hh) - 1
                  ) %>%
                  group_by({{ admin }}) %>%
                  filter({{ dom }} == 1, .preserve = TRUE) %>%
                  summarise(
            ...
            tab_P4_nr2 = tab_P4_nr1 %>%
              select(domain, admin, level, contains("everattended_new"))
            tab_P4_nr2 = tab_P4_nr2 %>%
              mutate(across(contains("everattended_new"), ~ as.double(NA)))
            names(tab_P4_nr2) = sub(
              "everattended_new_",
              "health_exp_hh_",
              names(tab_P4_nr2)
            )
...
time = time %>%
  add_row(., point = "Summaries", val = Sys.time(), diff = val - time$val[4])

#Process estimates for database
# source("./Estimate scripts/DDI estimate database.R")

time = time %>%
  add_row(., point = "Estimates", val = Sys.time(), diff = val - time$val[5])
