...
**********Tuvalu Population Census 2017**********
****Merging Deceased data with household data****
use "D:\DDI\Tuvalu\SPC_TUV_2017_PHC_Deceased_v01.dta", clear

egen death_hh=max(dyear==2017),by( island_code village_code ea_code hhldnum)

*(146 observations deleted)

duplicates drop island_code village_code ea_code hhldnum, force

*Duplicates in terms of island_code village_code ea_code hhldnum

*(4 observations deleted)

save "D:\DDI\Tuvalu\SPC_TUV_2017_PHC_Deceased_v01_2.dta", replace
use "D:\DDI\Tuvalu\SPC_TUV_2017_PHC_Household_v01.dta", clear
merge 1:m ea_code hhldnum using "D:\DDI\Tuvalu\SPC_TUV_2017_PHC_Deceased_v01_2.dta" 
/*  Result                           # of obs.
    -----------------------------------------
    not matched                         2,177
        from master                     2,177  (_merge==1)
        from using                          0  (_merge==2)

    matched                               216  (_merge==3)
    -----------------------------------------
*/
drop _merge
*gen death_hh=1
replace death_hh=0 if deceased_id==.

duplicates drop ea_code hhldnum,force
*(21 observations deleted)
****Merging individual data with household data****

merge 1:m ea_code hhldnum using "D:\DDI\Tuvalu\SPC_TUV_2017_PHC_Person_v01.dta"

/*  Result                           # of obs.
    -----------------------------------------
    not matched                         1,849
        from master                     1,103  (_merge==1)
        from using                        746  (_merge==2)

    matched                             9,542  (_merge==3)
    -----------------------------------------*/
/*Note: 1103- Institution member
		 746- Non household population*/
keep if _merge==3
*(1,849 observations deleted)
drop _merge
egen hh_id= concat(ea_code hhldnum), format(%25.0g) punct(_)
bysort hh_id: gen hh_size= _N

drop if age<15
*(3,166 observations deleted)

gen country_name="Tuvalu"
gen country_abrev="TUV" 
gen country_dataset_year="Tuvalu_Mini_Census_2017"
clonevar admin1=island_code 
clonevar admin2=village_code

egen ind_id= concat(ea_code hhldnum persnum), format(%25.0g) punct(_)
...
gen everattended_new=(curr_educ<4)
replace everattended_new=. if curr_educ==.

*Education - completed primary school

gen ind_atleastprimary_all = (level_completed>=11)
replace ind_atleastprimary_all =. if level_completed==.
replace ind_atleastprimary_all =0 if everattended_new==0 & age>=25

*Atleastprimary education

gen ind_atleastprimary = (level_completed>=11) if age>=25
replace ind_atleastprimary=. if level_completed==.
replace ind_atleastprimary =0 if everattended_new==0 & age>=25

*Atleastsecondary education

gen ind_atleastsecondary = (level_completed>=18) if age>=25
replace ind_atleastsecondary =. if level_completed==.
replace ind_atleastsecondary =0 if everattended_new==0 & age>=25

rename computer computer_asset
rename internet internet_access

gen computer=(computer_use==1)
gen internet=(internet_use ==1)
gen mobile_own=(mobile_use==1)

*Employment Status

gen ind_emp=(main_act<=9)
replace ind_emp=. if main_act==.

*Female at Managerial Work

gen work_managerial=cond(mi(occup1),.,cond(occup1==1,1,0))
...
gen work_manufacturing=cond(mi(indus1),.,cond(indus1==3,1,0))
replace work_manufacturing=. if ind_emp==0 | ind_emp==. 

*Informal Work

gen work_informal=cond(mi(main_act),.,cond(inlist(main_act, 6,7,8),1,0))
...
gen ind_water = inlist(watersource, 1,2,5)

*Sanitation

gen ind_toilet= inlist(toilettype, 1,2,3)

*Electricity

gen ind_electric=inlist(lighting, 1,2)

*Cooking fuel

gen ind_cleanfuel=inlist(cook, 1,2)

*Assets

gen ind_radio=(eradio==1)

gen ind_refrig=(refrigerator==1)

gen ind_computer=(computer_asset==1)

gen ind_tv=(tv==1)

gen ind_phone=(telephone==1)

egen cell_new=max(mobile_own), by(hh_id)

egen ind_asset_ownership=rowmean( ind_radio ind_phone ind_refrig ind_computer cell_new ind_tv)

*House rent

*gen house_rent=( tenure==2)

clonevar seeing_diff_new=seeing 
clonevar hearing_diff_new=hearing 
clonevar mobility_diff_new=walking 
clonevar cognitive_diff_new=remember  
...
replace deprive_sl_asset = 1 if (( ind_radio + ind_tv + ind_phone + ind_refrig) < 2)
replace deprive_sl_asset = . if ind_radio==. & ind_tv==. & ind_phone==. & ind_refrig==.
...
*gen not_owned_hh=(tenure!=1)
*replace not_owned_hh=. if tenure==9
gen alone=(hh_size==1)

save "D:\DDI\Tuvalu\Tuvalu Census 2017.dta", replace
...
replace ind_disaggvar_missing = (female==. | age==.) 

save "D:\DDI\Tuvalu\Tuvalu Census 2017 with missing.dta", replace
...
save "D:\DDI\Tuvalu\Tuvalu_Cleaned_Individual_Data_Trimmed.dta", replace

duplicates drop hh_id, force

save "D:\DDI\Tuvalu\Tuvalu_Cleaned_Household_Level_Data_Trimmed.dta", replace
