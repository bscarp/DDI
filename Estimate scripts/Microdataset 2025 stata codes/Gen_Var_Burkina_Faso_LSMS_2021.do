...
/*      Result                      Number of obs
    -----------------------------------------
    Not matched                             0
    Matched                             7,176  (_merge_asset==3)
    -----------------------------------------

*/


merge 1:1 grappe menage vague using "s08a_me_bfa2021.dta", generate(_merge_food)


/*    Result                      Number of obs
    -----------------------------------------
    Not matched                             0
    Matched                              7,176  (_merge_food==3)
    -----------------------------------------
*/
...
/*    Result                      Number of obs
    -----------------------------------------
    Not matched                             0
    Matched                             7,176  (_merge_health_exp==3)
    -----------------------------------------
*/
...
/*	Result                      Number of obs
    -----------------------------------------
    Not matched                            11
        from master                        11  (_merge_social_prot==1)
        from using                          0  (_merge_social_prot==2)

    Matched                             7,165  (_merge_social_prot==3)
    -----------------------------------------

*/

drop _merge_asset _merge_food _merge_shock _merge_health_exp _merge_social_prot
save "BFA_LSMS_2021_HH.dta", replace
*************************************************************************************************************************************************
use "ehcvm_individu_bfa2021.dta",clear

merge 1:1 grappe menage vague pid using "s04a_me_bfa2021.dta",gen(_merge_s04a)

/*     Result                      Number of obs
    -----------------------------------------
    Not matched                             0
    Matched                            47,090  (_merge_s04a==3)
    -----------------------------------------
...
count
merge 1:1 grappe menage vague pid using "s04b_me_bfa2021.dta",gen(_merge_s04b)

/*  Result                      Number of obs
    -----------------------------------------
    Not matched                             0
    Matched                            47,090  (_merge_s04b==3)
    -----------------------------------------

*/
keep if _merge_s04b==3
count
merge 1:1 grappe menage vague pid using "s03_me_bfa2021.dta", gen(_merge_s03)

/*  Result                      Number of obs
    -----------------------------------------
    Not matched                             0
    Matched                            47,090  (_merge_s03==3)
    -----------------------------------------
...
count
keep country year vague hhid grappe menage pid zae zaemil region province milieu hhweight resid sexe age alfa scol educ_hi telpor internet s04q06 s04q07 s04q08 s04q09 s04q18a s04q18b s04q28a s04q28b s04q30b s04q29b s04q39 s03q41 s03q42 s03q43 s03q44 s03q45 s03q46 s03q32 _merge_s04a _merge_s04b _merge_s03

save "BFA_LSMS_2021_Ind.dta", replace 

use "BFA_LSMS_2021_HH.dta",clear
merge 1:m grappe menage vague using "BFA_LSMS_2021_Ind.dta"

/*  Result                      Number of obs
    -----------------------------------------
    Not matched                             0
    Matched                            47,090  (_merge==3)
    -----------------------------------------
*/
*keep if vague==1
bysort hhid: gen hh_size = _N
save "Burkina Faso 2021 raw.dta", replace 

use "Burkina Faso 2021 raw.dta",clear 
drop if age<15
********Country Name****************
gen country_name="Burkina Faso"
********Country Abrevation*********
gen country_abrev="BF" 

*********Country_dataset_year*******
gen country_dataset_year="Burkina Faso LSMS 2021-2022"
...
egen ind_id= concat(hhid pid), format(%25.0g) punct(_)
...
save "Burkina_Faso_LSMS_2021.dta",replace 
...
save "Burkina_Faso_LSMS_2021 with missing.dta", replace
...
save "Burkina_Faso_LSMS_2021_Cleaned_Individual_Data_Trimmed.dta", replace

duplicates drop hh_id, force

save "Burkina Faso_LSMS_2021_Cleaned_Household_Level_Data_Trimmed.dta", replace
