...
use "D:\DDI\Bhutan\PHCB 2017\PHCB 2017\PHCB2017_HH_PUF.dta", clear
 merge 1:m houseid using "D:\DDI\Bhutan\PHCB 2017\PHCB 2017\PHCB_INDI_PUF.dta"
 keep if _merge==3
/*Result                           # of obs.
    -----------------------------------------
    not matched                        94,344
        from master                         0  (_merge==1)
        from using                     94,344  (_merge==2)

    matched                           632,801  (_merge==3)
    -----------------------------------------*/
*94,344 are Institution and Transient

merge m:1 houseid using "D:\DDI\Bhutan\MORTALITY_PUF_multiple death entery removed.dta", gen(_Merge_MORTALITY)

/* Result                           # of obs.
    -----------------------------------------
    not matched                             0
    matched                           632,831  (_Merge_MORTALITY==3)
    -----------------------------------------*/
drop if hl6>=2
*(38,768 observations deleted)
*household size
bysort houseid: gen hh_size = _N
save "D:\DDI\Bhutan\Bhutan census 2017 Raw.dta",replace
use "D:\DDI\Bhutan\Bhutan census 2017 Raw.dta",clear
 
 drop if hl5<15
*(168,142 observations deleted) 
gen country_name="Bhutan"

gen country_abrev="BT"

gen country_dataset_year="Bhutan Census 2017"

clonevar admin1=id1

clonevar admin2=gewog_town

rename houseid hh_id
rename personid ind_id
...
*Difficulties
	...
	clonevar seeing_diff_new= dm33
	replace seeing_diff_new=. if dm33==9
	clonevar hearing_diff_new= dm34
	replace hearing_diff_new=. if dm34==9
	clonevar mobility_diff_new= dm35
	replace mobility_diff_new=. if dm35==9
	clonevar cognitive_diff_new= dm36
	replace cognitive_diff_new=. if dm36==9
	clonevar selfcare_diff_new= dm37
	replace selfcare_diff_new=. if dm37==9
	clonevar comm_diff_new= dm38
	replace comm_diff_new=. if dm38==9
...
*Lit
	gen lit_new=( ee2==1 | ee3==1 | ee4==1 | ee5==1 )
    replace lit_new=. if ee2==9 & ee3==9 & ee4==9 & ee5==9
	replace lit_new=. if ee2==. & ee3==. & ee4==. & ee5==.
*Everattended School

	gen everattended_new=inlist(ee6, 1,2)
	replace everattended_new=. if ee6==9

*Education - completed primary school

	gen ind_atleastprimary_all =inrange(ee7, 6,17) | ee7==21
	replace ind_atleastprimary_all =. if ee7==.
	replace ind_atleastprimary_all =0 if everattended_new==0
	
*Atleastprimary education

	gen ind_atleastprimary =inrange(ee7, 6,17) | ee7==21
	replace ind_atleastprimary =. if ee7==. | age<25
	replace ind_atleastprimary =0 if everattended_new==0
	replace ind_atleastprimary =. if age<25
	
*Atleastsecondary education

	gen ind_atleastsecondary =inlist(ee7, 12,13,14,15,16,17,21)
	replace ind_atleastsecondary =. if ee7==. | age<25
...
*Managerial 
	...
	replace work_managerial2=1 if inrange(ee17, 1111,1432) & female==1 & ind_emp==1
	replace work_managerial2=. if ind_emp==. & (ee17==. & ee17==9999)

*Manufacturing

	gen work_manufacturing=(ee14==1113)
	replace work_manufacturing=. if ind_emp==0 | (ind_emp==. & ee14==.)

*Informal Work

	gen work_informal2=.
	replace work_informal2=0 if ind_emp==1
	replace work_informal2=1 if inlist(ee15, 2,4,5,6) & ind_emp==1
	replace work_informal2=. if ee15==. & ind_emp==.
*Youth Idle

	gen school_new=(ee6==1)
	replace school_new=. if ee6==9
...
*Electricity
	...
	gen ind_electric=inlist(hh7, 1,4)
	
*Cooking fuel

	gen ind_cleanfuel=inlist(hh8, 1,4,5)
	
*Water
	
	 gen ind_water=inlist(hh10, 1,2,3,4)
	
*Sanitation

	gen toilet_use=inlist(hh9, 1,2,3,4,7)
	gen ind_toilet=(toilet_use==1 & hh27==2)

** Adequate housing

*Floor 

	gen ind_floor=inlist(hh4, 2,5,6)
	
*Roof

	gen ind_roof=inlist(hh3, 3,6,7,8)
	
*Wall

	gen ind_wall=inlist(hh2, 6,7,8,9,10)

	gen ind_livingcond=(ind_floor==1&ind_roof==1&ind_wall==1)
*Assests

	*radio
		gen ind_radio=(hh13a==1)
		replace ind_radio=. if hh13a==9

	*Phone
		gen ind_phone=(hh13c==1)
		replace ind_phone=. if hh13c==9

	*Cell
		gen cell_new=(hh13d==1 | hh13e==1)
		replace cell_new=. if hh13d==9 & hh13e==9

	*Television
		gen ind_tv= ( hh13b==1)
		replace ind_tv=. if hh13b==9

	*Computer
		gen ind_computer=( hh13f==1 | hh13g==1)
		replace ind_computer=. if hh13f==9 & hh13g==9

	*Refrig
		gen ind_refrig=( hh14g==1)
		replace ind_refrig=. if hh14g==9

	*Motorcycle
		gen ind_motorcycle=(hh15c==1)
		replace ind_motorcycle=. if hh15c==9

	*Bicycle
		gen ind_bike=(hh15b==1)
		replace ind_bike=. if hh15b==9

	*Auto
		gen ind_autos=(hh14f==1)
		replace ind_autos=. if hh14f==9
	...
	*Food Insecurity
		...
		gen food_insecure=(hh26==1)
		replace food_insecure=. if hh26==9
	
gen alone=(hh_size==1)

gen death_hh=(mr1==1)
replace death_hh=. if mr1==.
...
save "D:\DDI\Bhutan\Bhutan_census_2017.dta", replace
...
save "D:\DDI\Bhutan\Bhutan_census_2017 with missing.dta", replace
...
save "D:\DDI\Bhutan\Bhutan_Census_2017_Cleaned_Individual_Data_Trimmed.dta", replace

duplicates drop hh_id, force

save "D:\DDI\Bhutan\Bhutan_Census_2017_Cleaned_Household_Level_Data_Trimmed.dta", replace
