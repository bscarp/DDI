...
cd "D:\DDI\Niger LSMS 2021\NER_2021_EHCVM-2_v01_M_STATA14"
*****************Niger LSMS 2021************************************************
*****************Household data Clean & merge***********************************
use "ehcvm_conso_ner2021.dta",clear
...
/*  Result                      Number of obs
    -----------------------------------------
    Not matched                             0
    Matched                             6,622  (_merge_asset==3)
    -----------------------------------------*/

merge 1:1 grappe menage vague using "s08a_me_ner2021.dta", generate(_merge_food)

/*  Result                      Number of obs
    -----------------------------------------
    Not matched                             0
    Matched                             6,622  (_merge_food==3)
    -----------------------------------------*/
merge 1:1 grappe menage vague using "shock.dta", generate(_merge_shock)
/*  Result                      Number of obs
    -----------------------------------------
    Not matched                             0
    Matched                             6,622  (_merge_shock==3)
    -----------------------------------------*/	
...
/*  Result                      Number of obs
    -----------------------------------------
    Not matched                             0
    Matched                             6,622  (_merge_health_exp==3)
    -----------------------------------------*/	
...
/*  Result                      Number of obs
    -----------------------------------------
    Not matched                             0
    Matched                             6,622  (_merge_social_prot==3)
    -----------------------------------------*/	
drop _merge_asset _merge_food _merge_shock _merge_health_exp _merge_social_prot
save "Niger_LSMS_2012_HH.dta", replace
*************************************************************************************************************************************************
use "ehcvm_individu_ner2021",clear

merge 1:1 grappe menage vague numind using "s04a_me_ner2021.dta",gen(_merge_s04a)

/*    Result                      Number of obs
    -----------------------------------------
    Not matched                         5,866
        from master                         0  (_merge_s04a==1)
        from using                      5,866  (_merge_s04a==2) (not the member of the household or missing)

    Matched                            38,753  (_merge_s04a==3)
    -----------------------------------------*/


count
merge 1:1 grappe menage vague numind using "s04b_me_ner2021.dta",gen(_merge_s04b)

/*      Result                      Number of obs
    -----------------------------------------
    Not matched                           539
        from master                       539  (_merge_s04b==1)
        from using                          0  (_merge_s04b==2)

    Matched                            44,080  (_merge_s04b==3)
    -----------------------------------------

*/
keep if _merge_s04b==3
count
merge 1:1 grappe menage vague numind using "s03_me_ner2021.dta", gen(_merge_s03)
/*     Result                      Number of obs
    -----------------------------------------
    Not matched                           539
        from master                         0  (_merge_s03==1)
        from using                        539  (_merge_s03==2)

    Matched                            44,080  (_merge_s03==3)
    -----------------------------------------
...
keep country year vague hhid grappe menage numind zae zaemil region departement commune milieu hhweight resid sexe age alfa scol educ_hi telpor internet s04q06 s04q07 s04q08 s04q09 s04q18a s04q18b s04q28a s04q28b s04q30b s04q29b s04q39 s03q41 s03q42 s03q43 s03q44 s03q45 s03q46 s03q32 _merge_s04a _merge_s04b _merge_s03

bysort hhid: gen hh_size = _N

save "Niger_LSMS_2012_Ind.dta", replace 

use "Niger_LSMS_2012_HH.dta",clear
merge 1:m grappe menage vague using "Niger_LSMS_2012_Ind.dta"

/*   Result                      Number of obs
    -----------------------------------------
    Not matched                             0
    Matched                            44,080  (_merge==3)
    -----------------------------------------


*/

save "Niger LSMS 2012 raw .dta", replace 
use "Niger LSMS 2012 raw .dta",clear
*keep if vague==1
drop if age<15
...
********Country Name****************
gen country_name="Niger"
********Country Abrevation*********
gen country_abrev="NE" 

*********Country_dataset_year*******
gen country_dataset_year="Niger LSMS 2020-2021"
...
save "D:\DDI\Niger LSMS 2021\Niger_LSMS_2021.dta",replace 
...
save "D:\DDI\Niger LSMS 2021\Niger_LSMS_2021 with missing.dta", replace
...
save "D:\DDI\Niger LSMS 2021\Niger_LSMS_2021_Cleaned_Individual_Data_Trimmed.dta", replace

duplicates drop hh_id, force

save "D:\DDI\Niger LSMS 2021\Niger_LSMS_2021_Cleaned_Household_Level_Data_Trimmed.dta", replace
