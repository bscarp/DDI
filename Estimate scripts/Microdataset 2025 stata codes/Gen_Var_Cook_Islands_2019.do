...
use "D:\DDI\Cook Island\SPC_COK_2019_LFS_Person_v01_PUF.dta", clear
merge m:1 anon_id07 using "D:\DDI\Cook Island\SPC_COK_2019_LFS_Household_v01_PUF.dta"
...
/*	Result                           # of obs.
    ...
    -----------------------------------------*/
save "D:\DDI\Cook Island\Cook Island LFS 2019 Merged.dta", replace
drop if age_grp5<=3
*(472 observations deleted)

gen country_name="Cook Island"

gen country_abrev="CK"

gen country_dataset_year="Cook Island LFS 2019"

clonevar admin1=anon_island


rename anon_id07 hh_id
rename pers_id ind_id

gen ind_weight=hh_weight
gen hh_weight=hh_weight

gen psu= anon_ea
gen sample_strata= urbanrural
*(From the report)
***Urban/Rural***

	gen urban_new=(urbanrural==1)
...
***Gender***
	...
	*Note: Age group at 5 intervals were avaiable
...
*Difficulties
	...
	replace seeing_diff_new=. if seeing==.
	clonevar hearing_diff_new= hearing
	replace hearing_diff_new=. if hearing==.
	clonevar mobility_diff_new= walking
	replace mobility_diff_new=. if walking==.
	clonevar cognitive_diff_new= remembering
	replace cognitive_diff_new=. if remembering==.
	clonevar selfcare_diff_new= selfcare
	replace selfcare_diff_new=. if selfcare==.
	clonevar comm_diff_new= communication
	replace comm_diff_new=. if communication==.
...
*Everattended School
	...
	gen everattended_new=(educ_completed>1)
	replace everattended_new=. if educ_completed==.


*Atleastprimary education

	gen ind_atleastprimary =(educ_completed>=4)
	replace ind_atleastprimary =. if educ_completed==. 
	replace ind_atleastprimary =. if age<6
	
*Atleastsecondary education

	gen ind_atleastsecondary =(educ_completed>=6)
	replace ind_atleastsecondary =. if educ_completed==. 
	replace ind_atleastsecondary =. if age<6

*Employment status

	gen ind_emp=(ilo_lfs==1)

   
*Managerial 

	gen work_managerial2=0 if female==1 & ind_emp==1
	replace work_managerial2=1 if isco1_main==1 & female==1 & ind_emp==1
	replace work_managerial2=. if ind_emp==. & isco1_main==. & isco1_main==11

*Manufacturing

	gen work_manufacturing=inlist(isic2_main, 10,11,14,18,23,27,30)
	replace work_manufacturing=. if ind_emp==0 | (ind_emp==. & isic2_main==. & isic2_main==22)

*Informal Work

	gen work_informal2=.
	replace work_informal2=0 if ind_emp==1
	replace work_informal2=1 if inlist(ilo_job1_ste_icse93, 3,5) & ind_emp==1
	replace work_informal2=. if ilo_job1_ste_icse93==. & ind_emp==.
	
*Youth Idle

	gen school_new=(currently_attending==1)
	replace school_new=. if currently_attending==.
...
*Social Protection
	...
	gen social_prot=(sp_cover==1)
	
*Food insecurity
	
	gen food_insecure= (worried==1 | healthy==1 | fewfood==1 | skipped==1 | ateless==1 | runout==1 | hungry==1 | whlday==1 )
	replace food_insecure=. if (worried==3 & healthy==3 & fewfood==3 & skipped==3 & ateless==3 & runout==3 & hungry==3 & whlday==3 )
	
*Living alone

	gen alone=(hhsize==1)
	
save "D:\DDI\Cook Island\Cook Island LFS 2019.dta", replace
...
save "D:\DDI\Cook Island\Cook Island LFS 2019 with missing.dta", replace
...
save "D:\DDI\Cook Island\Cook_Island_LFS_2019_Cleaned_Individual_Data.dta", replace

duplicates drop hh_id, force

save "D:\DDI\Cook Island\Cook_Island_LFS_2019_Cleaned_Household_Level_Data_Trimmed.dta", replace
