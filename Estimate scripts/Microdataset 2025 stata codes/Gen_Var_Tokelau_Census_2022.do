...
use "C:\Users\kavip\Downloads\SPC_TKL_2022_PHC_Household_v01.dta", clear
merge 1:m interview__key using "C:\Users\kavip\Downloads\SPC_TKL_2022_PHC_Person_v01.dta"
...
fre dwelling_type
keep if dwelling_type==1
tab data_type
keep if data_type==1
ta age5

drop if age5<4
fre seeing hearing walking remembering sefcare communication

save "D:\DDI\Tokelau\Tokelau.dta",replace

gen country_name="Tokelau"

gen country_abrev="TK"

gen country_dataset_year="Tokelau Census 2022"
...
tostring person_rost__id,replace
gen ind_id=interview__key+"_"+person_rost__id
...
clonevar selfcare_diff_new= sefcare 
...
gen lit_new=(literate_tok==1)
replace lit_new=1 if inlist(english_reading_skill, 3,4,5)&inlist(english_write_skill, 3,4,5)
replace lit_new=. if literate_tok==. & english_reading_skill==. & english_write_skill==. 

*Everattended School

gen everattended_new=(last_school_attend!=2)
replace everattended_new=. if last_school_attend==.

***Individual atleast Primary Education***

gen ind_atleastprimary_all=inrange(highest_qualification , 2, 12) 
replace ind_atleastprimary_all=0 if everattended_new==0 
replace ind_atleastprimary_all=. if highest_qualification==. & (everattended_new==.)

*Education - completed primary school

gen ind_atleastprimary=inrange(highest_qualification , 3, 12)
replace ind_atleastprimary=0 if everattended_new==0 
replace ind_atleastprimary=. if highest_qualification==. & (everattended_new==.)
replace ind_atleastprimary=. if age<5

***Individual atleast secondary Education***

gen ind_atleastsecondary=inrange(highest_qualification , 3, 12)
replace ind_atleastsecondary=0 if everattended_new==0 
replace ind_atleastsecondary=. if highest_qualification==. & (everattended_new==.)
replace ind_atleastsecondary=. if age<5

*Employment Status

gen ind_emp=(ilo_lfs_emp==1)
replace ind_emp=. if ilo_lfs_emp==.

*Female at Managerial Work

gen work_managerial2=0 if ind_emp==1 & female==1
replace work_managerial2= 1 if ind_emp==1 & ilo_major==1 & female==1
replace work_managerial2= . if (ind_emp==. & ilo_major==.) 
...
replace work_informal2=1 if inlist(employment_status,3,6)
replace work_informal2=1 if ind_emp==1 & unpaid_work==1
replace work_informal2=. if ind_emp==. & employment_status==. & unpaid_work==.

* Youth idle

gen school_new=(last_school_attend==1)
replace school_new=. if last_school_attend==.
...
replace youth_idle=. if age>6

rename internet internet_hh

*Internet

gen internet=(contact_internet!=5)
replace internet=. if contact_internet==.

*Own mobile

gen mobile_own=(contact_phone!=5)
replace mobile_own=. if contact_phone==.
...
gen ind_toilet=inlist(toilet_facility, 1,2)
replace ind_toilet=. if toilet_facility==.

*Drinking water Source

gen ind_water=inlist(drinking_water, 1,2,4)
replace ind_water=. if drinking_water==.

*Cooking fuel

gen ind_cleanfuel=inlist(means_cooking, 1,5)
replace ind_cleanfuel=. if means_cooking==.

*TV
gen ind_tv=(assets__2==1)

gen ind_phone=(assets__8==1)

gen ind_computer=(assets__3==1)

gen ind_refrig =(assets__5==1)

gen ind_autos=(assets__14==1)

egen ind_asset_ownership=rowmean(ind_tv ind_refrig ind_phone ind_computer ind_autos)
...
replace deprive_sl_asset = 1 if (( ind_tv + ind_phone + ind_refrig) < 2)
replace deprive_sl_asset = . if ind_tv==. & ind_phone==. & ind_refrig==. 
...
egen missing_sl=rowmiss( deprive_sl_fuel deprive_sl_asset)
replace missing_sl=4-missing_sl
egen sl_temp=rowtotal( deprive_sl_fuel deprive_sl_asset)
...
save "D:\DDI\Tokelau\Tokelau_Census_2022.dta",replace
...
save "D:\DDI\Tokelau\Tokelau_Census_2022_Cleaned_Individual_Data_Trimmed.dta", replace

duplicates drop hh_id, force

save "D:\DDI\Tokelau\Tokelau_Census_2022_Cleaned_Household_Level_Data_Trimmed.dta", replace
