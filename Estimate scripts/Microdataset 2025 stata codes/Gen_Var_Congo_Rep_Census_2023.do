...
cd "D:\DDI\Congo census 2023"

use "IndividusSample5percent.dta",clear
egen hh_id= concat(ID01 ID02 ID03 ID04 ID05 ID06 ID07 ID09 ID10 ID11 ID11 ID12 ID13), format(%25.0g) punct(_)
duplicates tag ID01 ID02 ID03 ID04 ID05 ID06 ID07 ID09 ID10 ID11 ID11 ID12 ID13 P01 , generate(dup)
tab dup
drop if dup==1
*(50 observations deleted)
bysort hh_id: gen hh_size = _N
save "IndividusSample5percent_wod.dta",replace

use "MenagesSample5percent.dta",clear
duplicates tag ID01 ID02 ID03 ID04 ID05 ID06 ID07 ID09 ID10 ID11 ID11 ID12 ID13, generate(dup)
tab dup
drop if dup==1
drop  dup
*(18 observations deleted)
merge 1:m ID01 ID02 ID03 ID04 ID05 ID06 ID07 ID09 ID10 ID11 ID11 ID12 ID13 using "IndividusSample5percent_wod.dta"
...
/*  Result                      Number of obs
    ...
    Matched                           280,141  (_merge==3)
    -----------------------------------------*/
	 drop if _merge==2
*(18 observations deleted)

save "Congo Census 2023 raw.dta",replace 
replace P06=. if P06==999


	drop if P06<15
	*(114,672 observations deleted)
	
	gen country_name="Congo"

	gen country_abrev="CG"

	gen country_dataset_year="Congo Rep Census 2023"

	*egen hh_id= concat(ID01 ID02 ID03 ID04 ID05 ID06 ID07 ID09 ID10 ID11 ID11 ID12 ID13), format(%25.0g) punct(_)
	egen ind_id= concat(ID01 ID02 ID03 ID04 ID05 ID06 ID07 ID09 ID10 ID11 ID11 ID12 ID13 P01), format(%25.0g) punct(_)
	duplicates tag ind_id, generate(dup_ind)
	gen ind_weight=1
	gen hh_weight=1
	clonevar admin1=ID01
	clonevar admin2=ID02
	*Urban/Rural

	gen urban_new=1 if ID07==1
	replace urban_new=0 if ID07==2
	
	*Gender

	gen female= 1 if (P04==2)
	replace female=0 if (P04==1)
	gen age=P06
	replace age=. if P06==.
...
*Difficulties
	...
	clonevar seeing_diff_new= HA1A 
	replace seeing_diff_new=. if  HA1A==9  
	clonevar hearing_diff_new= HA2A
	replace hearing_diff_new=. if HA2A==9
	clonevar mobility_diff_new= HA4A 
	replace mobility_diff_new=. if HA4A==9 
	clonevar cognitive_diff_new= HA6A 
	replace cognitive_diff_new=. if HA6A==9
	clonevar selfcare_diff_new= HA5A 
	replace selfcare_diff_new=. if HA5A==9 
	clonevar comm_diff_new= HA3A 
	replace comm_diff_new=. if HA3A==9
...
*Everattended School
	...
	gen everattended_new=inlist(FS01, 1,2)
	replace everattended_new=. if inlist(FS01, 8,9)
*Education - completed primary school
*This variable was created for computing multidimensional poverty
	gen ind_atleastprimary_all =inlist(FS06, 26) | inrange(FS06,31,34) |  inrange(FS06, 41,43) |  inrange(FS06, 71,78) |  inrange(FS06, 101,113)
	replace ind_atleastprimary_all=0 if everattended_new==0
	replace ind_atleastprimary_all =. if FS06==998
	
*Atleastprimary education

	gen ind_atleastprimary =inlist(FS06, 26) | inrange(FS06,31,34) |  inrange(FS06, 41,43) |  inrange(FS06, 71,78) |  inrange(FS06, 101,113)
	replace ind_atleastprimary =. if FS06==998
	replace ind_atleastprimary=0 if everattended_new==0
	replace ind_atleastprimary =. if age<=24 
	
*Atleastsecondary education

	gen ind_atleastsecondary = inrange(FS06, 71,78) |  inrange(FS06, 101,113)
	replace ind_atleastsecondary =. if FS06==998
	replace ind_atleastsecondary=0 if everattended_new==0
	replace ind_atleastsecondary =. if age<=24 
	
	gen education=0 if FS01==3
replace education=1 if inrange(FS06,0,34)| inrange(FS06,80,92) |  inrange(FS06, 41,43) 
replace education=2 if inrange(FS06, 71,78) |  inrange(FS06, 101,113)
	
*Lit_mew
	
	gen lit_new=( AL01!="X" | AL02!="X" )
	
*mobile	
	gen mobile_own = regexm(TIC1, "A|B")
	replace mobile_own =. if TIC1==" "
*computer
	gen computer = regexm(TIC2, "F")
	replace computer =. if TIC2==" "
*internet
	gen internet=(TIC5<10)
*(this question was asked for persons with mobile phone and hence persons with missing are considered as no access to internet)
...
*Managerial 
	...
	replace work_managerial2=1 if inrange(AE05A, 1111000,1349011) & female==1 & ind_emp==1
	replace work_managerial2=. if ind_emp==. & (AE05A==. & AE05A==9999999)

*Manufacturing

	gen work_manufacturing=inrange(AE08A, 100000, 320308)
	replace work_manufacturing=. if ind_emp==0 | (ind_emp==. & AE08A==.)

*Informal Work

	gen work_informal2=.
	replace work_informal2=0 if ind_emp==1
	replace work_informal2=1 if inlist(AE06, 2,7,8,9) & ind_emp==1
	replace work_informal2=1 if AE07==4 & ind_emp==1
	replace work_informal2=. if AE06==. & ind_emp==.
*Youth Idle

	gen school_new=(FS01==1)
	replace school_new=. if FS01==9 | FS01==8
...
*Wall
	...
	gen ind_wall=inlist(HM07, 34,35)
	replace ind_wall=. if HM07==. | HM07==99
	
*Roof

	gen ind_roof=inlist(HM08, 31,32,33)
	replace ind_roof=. if HM08==. | HM08==9
	
*Floor

	gen ind_floor=inlist(HM09,31, 32, 33, 34)
	replace ind_floor=. if HM09==. | HM08==9
...
*Water
	 ...
	 gen ind_water=inlist(HM11, 11,12,13,40,20,21,30,31,32,40,41,42,70,80)
	 replace ind_water=. if ind_water==99 | ind_water==.
	 
*Sanitation

	gen ind_toilet=inlist(HM15,11,12,13,21,22,24)
	replace ind_toilet=. if HM15==. | HM15==99
	
*Electricity

	gen ind_electric=inlist(HM12A, 1,2)
	replace ind_electric=. if HM12A==. | HM12A==99
*Cooking fuel

	gen ind_cleanfuel=inlist(HM13, 1,2)
	replace ind_cleanfuel=. if HM13==. | HM13==99
*assets

	gen ind_autos=(HM18C==1)
	replace ind_autos=. if HM18C==.
	gen ind_bike=(HM18A==1)
	replace ind_bike=. if HM18A==.
	gen ind_motorcycle=(HM18B==1)
	replace ind_motorcycle=. if HM18B==.
	gen ind_radio=(HM19C==1)
	replace ind_radio=. if HM19C==.
	gen ind_tv=(HM19E==1)
	replace ind_tv=. if HM19E==.
	gen ind_phone=(HM20A==1)
	replace ind_phone=. if HM20A==.
	gen cell_new=(HM20B==1)
	replace cell_new=. if HM20B==.
	gen ind_computer=(HM20F==1 | HM20E==1 )
	replace ind_computer=. if HM20F==. & HM20E==.
	*Refrig
	gen ind_refrig=(HM21B==1)
	replace ind_refrig=. if HM21B==.
...
save "Congo_Census_2023.dta",replace 
...
save "Congo_Census_2023 with missing.dta", replace
...
save "Congo_Census_2023_Cleaned_Individual_Data_Trimmed.dta", replace

duplicates drop hh_id, force

save "Congo_Census_2023_Cleaned_Household_Level_Data_Trimmed.dta", replace
