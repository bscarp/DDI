...
cd "D:\DDI\New Datasets\GNB_2021_EHCVM-2_v01_M_STATA14"

*****************New Datasets************************************************
*****************Household data Clean & merge***********************************
use "ehcvm_conso_gnb2021.dta",clear
...
/*      Result                      Number of obs
    -----------------------------------------
    Not matched                             0
    Matched                             5,351  (_merge_asset==3)
    -----------------------------------------
*/

merge 1:1 grappe menage vague using "s08a_me_gnb2021.dta", generate(_merge_food)
...
/*  Result                      Number of obs
    -----------------------------------------
    Not matched                             0
    Matched                             5,351  (_merge_social_prot==3)
    -----------------------------------------*/	
drop _merge_asset _merge_food _merge_shock _merge_health_exp _merge_social_prot
save "GNB_LSMS_2021_HH.dta", replace
*************************************************************************************************************************************************
use "ehcvm_individu_gnb2021.dta",clear
count
rename numind membres__id
merge 1:1 grappe menage vague membres__id using "s04a_me_gnb2021.dta",gen(_merge_s04a)

/*       Result                      Number of obs
    -----------------------------------------
    Not matched                         4,502
        from master                     4,502  (_merge_s04a==1)
        from using                          0  (_merge_s04a==2)

    Matched                            36,658  (_merge_s04a==3)
    -----------------------------------------

*/

count
merge 1:1 grappe menage vague membres__id using "s04b_me_gnb2021.dta",gen(_merge_s04b)

/*        Result                      Number of obs
    -----------------------------------------
    Not matched                         4,502
        from master                     4,502  (_merge_s04b==1)
        from using                          0  (_merge_s04b==2)

    Matched                            36,658  (_merge_s04b==3)
    -----------------------------------------

*/
count
merge 1:1 grappe menage vague membres__id using "s03_me_gnb2021.dta", gen(_merge_s03)

/*     Result                      Number of obs
    -----------------------------------------
    Not matched                            17
        from master                        17  (_merge_s03==1)
        from using                          0  (_merge_s03==2)

    Matched                            41,143  (_merge_s03==3)
    -----------------------------------------
*/

count
keep country year vague hhid grappe menage membres__id zae zaemil region prefecture milieu hhweight resid sexe age alfa scol educ_hi telpor internet s04q06 s04q07 s04q08 s04q09 s04q18a s04q18b s04q28a s04q28b s04q30b s04q29b s04q39 s03q41 s03q42 s03q43 s03q44 s03q45 s03q46 s03q32 _merge_s04a _merge_s04b _merge_s03

rename membres__id numind 
save "GNB_LSMS_2021_Ind.dta", replace 

use "GNB_LSMS_2021_HH.dta",clear
merge 1:m grappe menage vague using "GNB_LSMS_2021_Ind.dta"

/*    Result                      Number of obs
    -----------------------------------------
    Not matched                             0
    Matched                            41,160  (_merge==3)
    -----------------------------------------*/
*keep if vague==1
bysort hhid: gen hh_size = _N
save "Guinea Bisau 2021 raw.dta", replace 
*******************************************************************************************************************************
*________________________________________________________________________________________________________________________________*
**********************************************************************************************************************************
use "Guinea Bisau 2021 raw.dta",clear 
drop if age<15
********Country Name****************
gen country_name="Guinea Bisau"
********Country Abrevation*********
gen country_abrev="GW" 

*********Country_dataset_year*******
gen country_dataset_year="Guinea Bisau LSMS 2021-2022"
...
save "Guinea_Bisau_LSMS_2021.dta",replace 
...
save "Guinea_Bisau_LSMS_2021 with missing.dta", replace
...
save "Guinea_Bisau_LSMS_2021_Cleaned_Individual_Data_Trimmed.dta", replace

duplicates drop hh_id, force

save "Guinea_Bisau_LSMS_2021_Cleaned_Household_Level_Data_Trimmed.dta", replace
