...
cd "D:\DDI\New Datasets\BEN_2021_EHCVM-2_v01_M_STATA14"

*****************New Datasets************************************************
*****************Household data Clean & merge***********************************
use "ehcvm_conso_ben2021.dta",clear
bysort vague grappe menage : egen total_exp = total( depan )
gen exp_health = depan if coicop == 6 | inrange(codpr, 774,777)
replace exp_health =. if codpr==322  /*(removing 322. Lait, lotion de toilette corporelle as this being code as health expenese)*/
...
use "ehcvm_conso_ben2021.dta",clear
bysort vague grappe menage : egen total_exp = total( depan )
gen exp_health = depan if coicop == 6
bysort vague grappe menage: egen health_exp = total(exp_health)
duplicates drop vague grappe menage, force
save "health exp.dta",replace

use "s12_me_ben2021.dta", clear
...
/*     Result                      Number of obs
    -----------------------------------------
    Not matched                             0
    Matched                             8,032  (_merge_asset==3)
    -----------------------------------------

*/

merge 1:1 grappe menage vague using "s08a_me_ben2021.dta", generate(_merge_food)

/*       Result                      Number of obs
    -----------------------------------------
    Not matched                             0
    Matched                             8,032  (_merge_food==3)
    -----------------------------------------
...
/*    Result                      Number of obs
    -----------------------------------------
    Not matched                             0
    Matched                             8,032  (_merge_health_exp==3)
    -----------------------------------------
*/
...
/*    Result                      Number of obs
    -----------------------------------------
    Not matched                             0
    Matched                             8,032  (_merge_social_prot==3)
    -----------------------------------------
*/

save "ben_LSMS_2021_HH.dta", replace
*************************************************************************************************************************************************
use "ehcvm_individu_ben2021.dta",clear
rename numind membres__id
merge 1:1 grappe menage vague membres__id using "s04a_me_ben2021.dta",gen(_merge_s04a)

/*    Result                      Number of obs
    -----------------------------------------
    Not matched                         6,138
        from master                         0  (_merge_s04a==1)
        from using                      6,138  (_merge_s04a==2)

    Matched                            42,660  (_merge_s04a==3)
    -----------------------------------------*/

keep if _merge_s04a==3
count
merge 1:1 grappe menage vague membres__id using "s04b_me_ben2021.dta",gen(_merge_s04b)

/*    Result                      Number of obs
    -----------------------------------------
    Not matched                         6,138
        from master                         0  (_merge_s04b==1)
        from using                      6,138  (_merge_s04b==2)

    Matched                            42,660  (_merge_s04b==3)
    -----------------------------------------*/


keep if _merge_s04b==3
count
merge 1:1 grappe menage vague membres__id using "s03_me_ben2021.dta", gen(_merge_s03)
/*    Result                      Number of obs
    -----------------------------------------
    Not matched                         6,138
        from master                         0  (_merge_s03==1)
        from using                      6,138  (_merge_s03==2)

    Matched                            42,660  (_merge_s03==3)
    -----------------------------------------*/

keep if _merge_s03==3
count
...
save "BEN_LSMS_2021_Ind.dta", replace 

use "BEN_LSMS_2021_HH.dta",clear
merge 1:m grappe menage vague using "BEN_LSMS_2021_Ind.dta"
...
save "Benin 2021 raw.dta", replace 

use "Benin 2021 raw.dta",clear

drop if age<15
********Country Name****************
gen country_name="Benin"
********Country Abrevation*********
gen country_abrev="BJ" 

*********Country_dataset_year*******
gen country_dataset_year="Benin LSMS 2021-2022"
...
save "Benin_LSMS_2021.dta",replace 
...
save "Benin_LSMS_2021 with missing.dta", replace
...
save "Benin_LSMS_2021_Cleaned_Individual_Data_Trimmed.dta", replace

duplicates drop hh_id, force

save "Benin_LSMS_2021_Cleaned_Household_Level_Data_Trimmed.dta", replace
