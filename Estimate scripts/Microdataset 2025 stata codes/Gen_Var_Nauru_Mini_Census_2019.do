...
**********Nauru Mini Census 2019**********
use "D:\DDI\Nauru\SPC_NRU_2019_PHC_Person_v01.dta", clear
gen hh_id= interview_key
bysort hh_id: gen hh_size= _N
drop if age<15
*(4,306 observations deleted)

gen country_name="Nauru"
gen country_abrev="NAU" 
gen country_dataset_year="Nauru-Mini-Census-2019"
clonevar admin1=district 
*clonevar admin2=

egen ind_id= concat(interview_key person_id), format(%25.0g) punct(_)
...
gen everattended_new=(ever_attended==1)

*Education - completed primary school

gen ind_atleastprimary_all = (level_attended>=3)
replace ind_atleastprimary_all =. if level_attended==.
replace ind_atleastprimary_all =0 if everattended_new==0

*Atleastprimary education

gen ind_atleastprimary = (level_attended>=3) if age>=25
replace ind_atleastprimary=. if level_attended==.
replace ind_atleastprimary =0 if everattended_new==0 & age>=25

*Atleastsecondary education

gen ind_atleastsecondary = (level_attended>=5) if age>=25
replace ind_atleastsecondary =. if level_attended==.
replace ind_atleastsecondary =0 if everattended_new==0 & age>=25

gen ind_emp=inlist(main_activity, 2,3,4,7)

rename internet internet_use
...
gen school_new=(sch_attendance==1)
...
clonevar comm_diff_new=comunication
...
save "D:\DDI\Nauru\Nauru Mini Census 2019.dta", replace
...
save "D:\DDI\Nauru\Nauru Mini Census 2019 with missing.dta", replace
...
save "D:\DDI\Nauru\Nauru_Mini_Census_2019_Cleaned_Individual_Data_Trimmed.dta", replace

duplicates drop hh_id, force

save "D:\DDI\Nauru\Nauru_Mini_Census_2019_Household_Level_Data_Trimmed.dta", replace

su disability_any_hh disability_some_hh disability_atleast_hh disability_alot_hh disability_unable_hh
