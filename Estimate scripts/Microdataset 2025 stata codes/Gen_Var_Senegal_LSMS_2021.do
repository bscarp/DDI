...
cd "D:\DDI\New Datasets\SEN_2021_EHCVM-2_v01_M_STATA14"

*****************New Datasets************************************************
*****************Household data Clean & merge***********************************
use "ehcvm_conso_sen2021.dta",clear
...
use "s11_me_sen2021.dta", clear

keep grappe menage vague s11q18 s11q19 s11q20 s11q26a s11q26b s11q37 s11q52__4 s11q52__5 s11q54 s11q55

merge 1:1 grappe menage using "s00_me_sen2021.dta",gen(_merge_cov)

/*   Result                      Number of obs
    -----------------------------------------
    Not matched                             0
    Matched                             7,120  (_merge_cov==3)
    -----------------------------------------
*/
...
/*     Result                      Number of obs
    -----------------------------------------
    Not matched                             0
    Matched                             7,120  (_merge_asset==3)
    -----------------------------------------

*/

merge 1:1 grappe menage vague using "s08a_me_sen2021.dta", generate(_merge_food)

/*       Result                      Number of obs
    -----------------------------------------
    Not matched                             0
    Matched                             7,120 (_merge_food==3)
    -----------------------------------------
...
/*    Result                      Number of obs
    -----------------------------------------
    Not matched                             0
    Matched                             7,120  (_merge_health_exp==3)
    -----------------------------------------
*/
...
/*    Result                      Number of obs
    -----------------------------------------
    Not matched                             0
    Matched                             7,120  (_merge_social_prot==3)
    -----------------------------------------
*/

save "SEN_LSMS_2021_HH.dta", replace
*************************************************************************************************************************************************
use "ehcvm_individu_sen2021.dta",clear
rename numind membres__id
merge 1:1 grappe menage vague membres__id using "s04a_me_sen2021.dta",gen(_merge_s04a)

/*        Result                      Number of obs
    -----------------------------------------
    Not matched                         9,004
        from master                     9,004  (_merge_s04a==1)
        from using                          0  (_merge_s04a==2)

    Matched                            54,526  (_merge_s04a==3)
    -----------------------------------------
...
count
merge 1:1 grappe menage vague membres__id using "s04b_me_sen2021.dta",gen(_merge_s04b)

/*        Result                      Number of obs
    -----------------------------------------
    Not matched                        41,371
        from master                    41,371  (_merge_s04b==1)
        from using                          0  (_merge_s04b==2)

    Matched                            22,159  (_merge_s04b==3)
    -----------------------------------------
*/
...
count
merge 1:1 grappe menage vague membres__id using "s03_me_sen2021.dta", gen(_merge_s03)
/*      Result                      Number of obs
    -----------------------------------------
    Not matched                         1,316
        from master                     1,316  (_merge_s03==1)
        from using                          0  (_merge_s03==2)

    Matched                            62,214  (_merge_s03==3)
    -----------------------------------------
...
save "SEN_LSMS_2021_Ind.dta", replace 

use "SEN_LSMS_2021_HH.dta",clear
merge 1:m grappe menage vague using "SEN_LSMS_2021_Ind.dta"
...
save "Senegal 2021 raw.dta", replace 

use "Senegal 2021 raw.dta",clear
...
gen country_dataset_year="Senegal LSMS 2021-2022"
...
save "Senegal_LSMS_2021.dta",replace 
...
save "Senegal_LSMS_2021 with missing.dta", replace
...
save "Senegal_LSMS_2021_Cleaned_Individual_Data_Trimmed.dta", replace

duplicates drop hh_id, force

save "Senegal_LSMS_2021_Cleaned_Household_Level_Data_Trimmed.dta", replace
