...
import spss using "D:\DDI\Tanzania 2022\TPHC2022_TenPercent_Death\TPHC2022_TenPercent_Death.sav", clear
duplicates drop H_REGION_NAME H_DISTRICT_NAME H_COUNCIL_NAME H_CONSTITUENCY_NAME H_DIVISION_NAME H_WARD_NAME H_VILLAGE_NAME H_HAMLET_NAME PHHN,force
keep H_REGION_NAME H_DISTRICT_NAME H_COUNCIL_NAME H_CONSTITUENCY_NAME H_DIVISION_NAME H_WARD_NAME H_VILLAGE_NAME H_HAMLET_NAME PHHN DEATHS_ANY DEATHS
merge 1:m H_REGION_NAME H_DISTRICT_NAME H_COUNCIL_NAME H_CONSTITUENCY_NAME H_DIVISION_NAME H_WARD_NAME H_VILLAGE_NAME H_HAMLET_NAME PHHN using "D:\DDI\Tanzania 2022\TPHC2022_TenPercent\TPHC2022_TenPercent.dta"


/*   Result                      Number of obs
    -----------------------------------------
    Not matched                     5,915,974
        from master                         0  (_merge==1)
        from using                  5,915,974  (_merge==2)

    Matched                           265,798  (_merge==3)
    -----------------------------------------*/



fre DEATHS_ANY DEATHS
replace DEATHS_ANY=0 if DEATHS_ANY==.
replace DEATHS=0 if DEATHS==.
fre DEATHS_ANY DEATHS
save "D:\DDI\Tanzania 2022\TPHC2022_TenPercent\Tanzania_census_2022_raw_merged.dta",replace 
use "D:\DDI\Tanzania 2022\TPHC2022_TenPercent\Tanzania_census_2022_raw_merged.dta", clear
*Keeping Only Households*
keep if GQ==2

egen hh_id= concat( PREGION PDISTRICT PCOUNCIL PCONSTITUENCY PDIVISION PWARD PVILLAGE PHAMLET PHHN ), format(%25.0g) punct(0)
*household size
bysort hh_id: gen hh_size = _N
gen per_number = .
bysort hh_id (hh_id): replace per_number = _n
egen ind_id= concat( PREGION PDISTRICT PCOUNCIL PCONSTITUENCY PDIVISION PWARD PVILLAGE PHAMLET PHHN per_number), format(%25.0g) punct(0)

*destring hh_id ind_id,replace
drop if AGE<15

gen country_name="Tazania"

gen country_abrev="TZA"

gen country_dataset_year="Tazania Census 2022"

clonevar admin1=PREGION
clonevar admin2=H_DISTRICT_NAME
gen ind_weight=1
gen hh_weight=1

***Urban/Rural***

gen urban_new=1 if LOCATION==2
replace urban_new=0 if LOCATION==1
...
clonevar selfcare_diff_new=SELF_CARE 
clonevar comm_diff_new=COMMUNICATING
...
gen everattended_new = (SCHOOLING<=3)
replace everattended_new=. if SCHOOLING==.

***Literacy***

gen lit_new=(LITERATE<=4)
replace lit_new=. if LITERATE==.

***Individual atleast Primary Education***

gen ind_atleastprimary_all=inrange(EDUC_LEVEL , 6, 20) 
replace ind_atleastprimary_all=0 if everattended_new==0 
replace ind_atleastprimary_all=. if EDUC_LEVEL==. & (SCHOOLING<=3)

*Education - completed primary school

gen ind_atleastprimary=inrange(EDUC_LEVEL , 6, 20) 
replace ind_atleastprimary=0 if everattended_new==0 
replace ind_atleastprimary=. if EDUC_LEVEL==. & (SCHOOLING<=3)
replace ind_atleastprimary=. if age<25

***Individual atleast secondary Education***

gen ind_atleastsecondary=inlist(EDUC_LEVEL , 12,13,14, 15, 17) 
replace ind_atleastsecondary=0 if everattended_new==0 
replace ind_atleastsecondary=. if EDUC_LEVEL==. & (SCHOOLING<=3)
replace ind_atleastsecondary=. if age<25

gen education=0 if (SCHOOLING>3)
replace education=1 if inrange(EDUC_LEVEL , 0, 20)
replace education=2 if inlist(EDUC_LEVEL , 12,13,14, 15, 17) 

*Employment Status

gen ind_emp=(ECONOMIC_ACTIVITY<=4)
replace ind_emp=. if ECONOMIC_ACTIVITY==.

*Female at Managerial Work

gen work_managerial2=0 if ind_emp==1 & female==1
replace work_managerial2= 1 if ind_emp==1 & TASCO_CODE==1 & female==1
replace work_managerial2= . if (ind_emp==. & TASCO_CODE==.) 

*Manufacturing Worker

gen work_manufacturing=cond(mi(INDUSTRY_CODE),.,cond(INDUSTRY_CODE==3,1,0))
...
replace work_informal2=1 if inlist(ECONOMIC_ACTIVITY,3,4)
replace work_informal2=1 if ind_emp==1 & (inlist(EMPLOYMENT_STATUS, 2,4,5) | inlist(OWNER, 8,9,10))
replace work_informal2=. if ind_emp==. & EMPLOYMENT_STATUS==. & OWNER==.

* Youth idle

gen school_new=(SCHOOLING==1)
replace school_new=. if SCHOOLING==.
...
gen health_insurance=(INSURANCE==1)
replace health_insurance=. if INSURANCE==9

*Electricity

gen ind_electric=inlist(LIGHTING, 1,2,13)
replace ind_electric=. if LIGHTING==.

*Cooking fuel

gen ind_cleanfuel=inrange(COOKING, 1,6)
replace ind_cleanfuel=. if COOKING==99

*Drinking water Source

gen ind_water=inlist(DRINKING_WATER, 1,2,3,4,5,6,8,10,11)
replace ind_water=. if DRINKING_WATER==.

*Sanitation

gen ind_toilet=inlist(TOILET, 1,2,3,5,6,7,8)
replace ind_toilet=. if TOILET==.

*Adquate housing

*Roof

gen ind_roof=inrange(ROOF, 1,3)
replace ind_roof=. if ROOF==.

*Floor

gen ind_floor=inrange(FLOOR, 1,5)
replace ind_floor=. if FLOOR==.

*Wall

gen ind_wall=inlist(WALLS, 2,4)
replace ind_wall=. if WALLS==.
...
*Radio
gen ind_radio = .
replace ind_radio = 1 if regexm(ASSETS, "[A]") // Check if string contains 'A' 
replace ind_radio = 0 if regexm(ASSETS, "[A-Za-z]") & !regexm(ASSETS, "[A]") // Contains letters but no 'A'

*Television
gen ind_tv = .
replace ind_tv = 1 if regexm(ASSETS, "[I]") 
replace ind_tv = 0 if regexm(ASSETS, "[A-Za-z]") & !regexm(ASSETS, "[I]") 

*Telephone
gen ind_phone = .
replace ind_phone = 1 if regexm(ASSETS, "[B]") 
replace ind_phone = 0 if regexm(ASSETS, "[A-Za-z]") & !regexm(ASSETS, "[B]") 

*Cell
gen cell_new = .
replace cell_new = 1 if regexm(ASSETS, "[C]")  
replace cell_new = 0 if regexm(ASSETS, "[A-Za-z]") & !regexm(ASSETS, "[C]")

*Bike
gen ind_bike = .
replace ind_bike = 1 if regexm(ASSETS, "[D]") 
replace ind_bike = 0 if regexm(ASSETS, "[A-Za-z]") & !regexm(ASSETS, "[D]") 

*Motorcycle
gen ind_motorcycle = .
replace ind_motorcycle = 1 if regexm(ASSETS, "[F]")
replace ind_motorcycle = 0 if regexm(ASSETS, "[A-Za-z]") & !regexm(ASSETS, "[F]")

*Refrigrator
gen ind_refrig = .
replace ind_refrig = 1 if regexm(ASSETS, "[M]")
replace ind_refrig = 0 if regexm(ASSETS, "[A-Za-z]") & !regexm(ASSETS, "[M]")

*Computer
gen ind_computer = .
replace ind_computer = 1 if regexm(ASSETS, "[N]")
replace ind_computer = 0 if regexm(ASSETS, "[A-Za-z]") & !regexm(ASSETS, "[N]") 

*Assets ownership

egen ind_asset_ownership=rowmean(ind_radio ind_tv ind_refrig ind_phone cell_new ind_computer ind_bike ind_motorcycle)

*HH death_hh

gen death_hh=(DEATHS==1)
...
replace deprive_sl_asset = 1 if (( ind_radio + ind_tv + ind_phone + ind_refrig + ind_bike + ind_motorcycle) < 2)
replace deprive_sl_asset = . if ind_radio==. & ind_tv==. & ind_phone==. & ind_refrig==. & ind_bike==. & ind_motorcycle==. 
...
save "D:\DDI\Tanzania 2022\Tanzania_Census_2022.dta",replace
...
save "D:\DDI\Tanzania 2022\Tanzania_Census_2022 with missing.dta", replace
...
save "D:\DDI\Tanzania 2022\Tanzania_Census_2022_Cleaned_Individual_Data_Trimmed.dta", replace

duplicates drop hh_id, force

save "D:\DDI\Tanzania 2022\Tanzania_Census_2022_Cleaned_Household_Level_Data_Trimmed.dta", replace
