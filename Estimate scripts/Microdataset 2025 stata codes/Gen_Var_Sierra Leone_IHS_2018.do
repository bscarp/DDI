...
use "D:\DDI\Albania\SSAPOV-data\SLE_2018_SLIHS_v01_M_v01_A_SSAPOV_I.dta" ,clear
merge 1:1 pid hid using "D:\DDI\Albania\SSAPOV-data\SLE_2018_SLIHS_v01_M_v01_A_SSAPOV_L.dta", gen(_merge_L)

/*    Result                           # of obs.
    -----------------------------------------
    not matched                             0
    matched                            40,462  (_merge_L==3)
    -----------------------------------------*/
merge 1:1 pid using "D:\DDI\sierra leone\Sierra Leone IHS 2018 mobile computer internet.dta", gen(_merge_mobile)
*Extracted mobile use internet use and computer use variables from raw data and merged.
/*    Result                           # of obs.
    -----------------------------------------
    not matched                           146
        from master                         0  (_merge_mobile==1)
        from using                        146  (_merge_mobile==2)

    matched                            40,462  (_merge_mobile==3)
    -----------------------------------------*/
keep if _merge_mobile==3
*(146 observations deleted)
 merge m:1 hid using "D:\DDI\Albania\SSAPOV-data\SLE_2018_SLIHS_v01_M_v01_A_SSAPOV_H.dta", gen(_merge_H)

/*    Result                           # of obs.
    -----------------------------------------
    not matched                             0
    matched                            40,462  (_merge_H==3)
    -----------------------------------------*/

merge m:1 hid using "D:\DDI\Albania\SSAPOV-data\SLE_2018_SLIHS_v01_M_v01_A_SSAPOV_P.dta", gen(_merge_P)
*(note: variable wta_hh was float, now double to accommodate using data's values)

/*    Result                           # of obs.
    -----------------------------------------
    not matched                             0
    matched                            40,462  (_merge_P==3)
    -----------------------------------------*/
merge m:1 hid using "D:\DDI\sierra leone\Sierra Leone IHS 2018 Food inscurity hh.dta", gen(_merge_food)
*Extracted food insurity variables from raw data and merged.
/*	Result                           # of obs.
    -----------------------------------------
    not matched                            30
        from master                         0  (_merge_food==1)
        from using                         30  (_merge_food==2)

    matched                            40,462  (_merge_food==3)
    -----------------------------------------*/
keep if _merge_food==3
merge m:1 hhno cluster using "D:\DDI\sierra leone\Sierra Leone IHS 2018 social protection.dta", gen (_merge_social)
/*   Result                      Number of obs
    -----------------------------------------
    Not matched                            45
        from master                        15  (_merge_social==1)
        from using                         30  (_merge_social==2)

    Matched                            40,447  (_merge_social==3)
    -----------------------------------------*/


drop if _merge_social==2
*(30 observations deleted)

drop _merge_L _merge_mobile _merge_H _merge_P _merge_food _merge_social

save "D:\DDI\sierra leone\Sierra Leone IHS 2018 Raw.dta",replace

drop if ageyr<15
*(16,716 observations deleted)
drop if eye_dsablty==. & hear_dsablty==. & walk_dsablty==. & conc_dsord==. & slfcre_dsablty==. & comm_dsablty==.
*(926 observations deleted)

gen country_name="Sierra Leone"

gen country_abrev="SL"

gen country_dataset_year="Sierra Leone IHS 2018"

*clonevar admin1=region1
gen admin1 = substr(region1, strpos(region1, "-") + 1, .)
*clonevar admin2=region2
gen admin2 = substr(region2, strpos(region2, "-") + 1, .)

gen hh_id=hid 
egen ind_id= concat( hh_id pid ), format(%25.0g) punct(0)

gen ind_weight=wta_pop
gen hh_weight=wta_hh
gen sample_strata = real(substr(strata , 1, 3))
gen psu=cluster

***Urban/Rural***

	clonevar urban_new=rururb
***Gender***
...
*Difficulties
	...
	clonevar seeing_diff_new= eye_dsablty
	clonevar hearing_diff_new= hear_dsablty
	clonevar mobility_diff_new= walk_dsablty
	clonevar cognitive_diff_new= conc_dsord
	clonevar selfcare_diff_new= slfcre_dsablty
	clonevar comm_diff_new= comm_dsablty
...
*Lit
	...
	gen lit_new=literacy

*Everattended School

	gen everattended_new=everattd

*Education - completed primary school

gen ind_atleastprimary_all =(educat10>=4)
replace ind_atleastprimary_all =0 if everattended_new==0
replace ind_atleastprimary_all =. if (educat10==. & everattd==.) | (educat10==. & everattd>=1)

*Atleastprimary education

gen ind_atleastprimary =(educat10>=4) if age>=25
replace ind_atleastprimary =0 if everattended_new==0 & age>=25
replace ind_atleastprimary =. if (educat10==. & everattd==.) | (educat10==. & everattd>=1) 


*Atleastsecondary education

gen ind_atleastsecondary =(educat10>=6) if age>=25
replace ind_atleastsecondary =0 if everattended_new==0 & age>=25
replace ind_atleastsecondary =. if (educat10==. & everattd==.) | (educat10==. & everattd>=1) 

*Mobile own

gen mobile_own=(c22==1)
replace mobile_own=. if c22==.
...
gen computer=(c26==1)
replace computer=. if c26==.

*Internet use

gen internet=(c27==1)
replace internet=. if c27==.

*Employment status

gen ind_emp=(lstatus==1)
replace ind_emp=. if lstatus==.

*Managerial 

gen work_managerial2=0 if female==1 & ind_emp==1
replace work_managerial2=1 if inrange(h13c, 111,143) & female==1 & ind_emp==1
replace work_managerial2=. if ind_emp==. & h13c==.

*Manufacturing

gen work_manufacturing=(industrycat10==3)
replace work_manufacturing=. if ind_emp==0 | (ind_emp==. & industrycat10==.)

*Informal Work

gen work_informal2=.
replace work_informal2=0 if ind_emp==1
replace work_informal2=1 if inlist(empstat, 2,4,5) & ind_emp==1
replace work_informal2=1 if (empstat==1 & contract==0) & ind_emp==1
replace work_informal2=. if empstat==. & ind_emp==.

*Youth Idle

gen school_new = atschool
...
gen food_insecure =.
replace food_insecure=1 if (m13==1 | m14a>=1 | m14b>=1 | m14c>=1 | m14d>=1 | m14e>=1 | m14f>=1 | m14g>=1 | m14h>=1)
replace food_insecure=0 if (m13==2 & m14a==0 & m14b==0 & m14c==0 & m14d==0 & m14e==0 & m14f==0 & m14g==0 & m14h==0)

gen alone=(hhsize==1)

*water
gen ind_water=inlist(watercs,"1 - piped","2 - tubewell / borehole","3 - protected dug well","5 - protected spring","7 - rain water","8 - surface water","9 - bottle / sachet water")
replace ind_water=. if watercs==""

*sanitation
gen toilet_use=inlist(toiletcs, "1 – flush to piped sewer system","2 – flush to septic tank","3 – flush to pit (latrine)","5 – ventilated improved pit latrine (vip)","6 – pit latrine with slab","8 – composting toilet")
replace toilet_use=. if toiletcs==""

gen ind_toilet=(toilet_use==1 & toiletshared==0)
replace ind_toilet=. if toilet_use==. & toiletshared==.

*electricity
gen ind_electric=inlist(fuellighcs, "3 – mains electricity (edsa, npa, bkp)","5 – solar panels")
replace ind_electric=. if fuellighcs==""

*cooking fuel
gen ind_cleanfuel=inlist(fuelcookcs, "4 – gas","5 – electricity")
replace ind_cleanfuel=. if fuelcookcs==""

*Assets
*radio
gen ind_radio=radio
*cell
gen cell_new=cellphone
**Phone
gen ind_phone=phone
*TV
gen ind_tv=television
*Motorcycle
gen ind_motorcycle=mcycle
* Bike
gen ind_bike=bcycle
*computer
gen ind_computer=computer_hh
*Auto
gen ind_autos=car
*refridge
gen ind_refrig=fridge
...
egen social_prot = rowmin(m2101 m2102 m2103 m2104 m2105 m2106 m2107 m2108)

*Housing Condition
*Wall

gen ind_wall= (wallcs=="5 – cement blocks")
replace ind_wall=. if wallcs==""

*Floor

gen ind_floor=(floorcs=="4 – concrete" | floorcs=="2 – tile")
replace ind_floor=. if floorcs==""

*Roof

gen ind_roof=inlist(roofcs, "2 – corrugated iron sheets","3 – cement / concrete ","4 – roofing tiles")
replace ind_roof=. if roofcs==""
...
save "D:\DDI\sierra leone\Sierra Leone IHS 2018.dta",replace
...
save "D:\DDI\sierra leone\Sierra Leone IHS 2018 with missing.dta", replace

drop if ind_func_diff_missing==1 | ind_disaggvar_missing==1

save "D:\DDI\sierra leone\Sierra_Leone_IHS_2018.dta", replace
...
save "D:\DDI\sierra leone\Sierra_Leone_IHS_2018_Cleaned_Individual_Data.dta", replace

duplicates drop hh_id, force

save "D:\DDI\sierra leone\Sierra_Leone_IHS_2018_Cleaned_Household_Level_Data_Trimmed.dta", replace
