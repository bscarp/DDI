...
*/
cd "D:\DDI\New Datasets\CIV_2021_EHCVM-2_v01_M_STATA14"

*****************New Datasets************************************************
*****************Household data Clean & merge***********************************
use "ehcvm_conso_civ2021.dta",clear
bysort  grappe menage : egen total_exp = total( depan )
gen exp_health = depan if coicop == 6| inrange(codpr, 774,777)
bysort  grappe menage: egen health_exp = total(exp_health)

duplicates drop  grappe menage, force
save "health exp.dta",replace


use "s12_me_civ2021.dta", clear
...
use "s08a_me_civ2021.dta",clear

duplicates drop grappe menage,force

save "s08a_me_civ2021_rdup.dta",replace

use "s14b_me_civ2021.dta",clear

collapse (first) s14bq02, by(grappe menage s14bq01)
*removing duplicates
  reshape wide s14bq02, i(grappe menage) j(s14bq01)
  
save "shock.dta",replace

use "s15_me_civ2021.dta", clear
 keep grappe menage vague s15q01 s15q02
 reshape wide s15q02 , i(grappe menage vague) j( s15q01 )
 drop if s15q021==.
save "social_prot.dta",replace

use "s11_me_civ2021.dta", clear

keep grappe menage s11q18 s11q19 s11q20 s11q26a s11q26b s11q37 s11q52__4 s11q52__5 s11q54 s11q55

merge 1:1 grappe menage using "s00_me_civ2021.dta",gen(_merge_cov)
    /*Result                      Number of obs
    -----------------------------------------
    Not matched                           728
        from master                         0  (_merge_cov==1)
        from using                        728  (_merge_cov==2)

    Matched                            12,965  (_merge_cov==3)
    -----------------------------------------*/


keep if _merge_cov==3
merge 1:1 grappe menage using "assets.dta", generate(_merge_asset)

/*      Result                      Number of obs
    -----------------------------------------
    Not matched                             0
    Matched                             12,965  (_merge_asset==3)
    -----------------------------------------
*/
*keep if _merge_asset==3
merge 1:1 grappe menage using "s08a_me_civ2021_rdup.dta", generate(_merge_food)

/*     Result                      Number of obs
    -----------------------------------------
    Not matched                             1
        from master                         0  (_merge_food==1)
        from using                          1  (_merge_food==2)

    Matched                            12,965  (_merge_food==3)

    -----------------------------------------
*/
*keep if _merge_asset==3	
merge 1:1 grappe menage  using "shock.dta", generate(_merge_shock)

/*    Result                      Number of obs
    -----------------------------------------
    Not matched                         1,128
        from master                       206  (_merge_shock==1)
        from using                        922  (_merge_shock==2)

    Matched                            12,759  (_merge_shock==3)
    -----------------------------------------
*/

drop if _merge_shock==2		
	
merge 1:1 grappe menage using "health exp.dta", generate(_merge_health_exp)


/*    Result                      Number of obs
    -----------------------------------------
    Not matched                             0
    Matched                            12,965  (_merge_health_exp==3)
    -----------------------------------------
*/
...
/*    Result                      Number of obs
    -----------------------------------------
    Not matched                             0
    Matched                            12,965  (_merge_social_prot==3)
    -----------------------------------------
*/
drop _merge_asset _merge_food _merge_shock
save "CIV_LSMS_2021_HH.dta", replace
*************************************************************************************************************************************************
use "ehcvm_individu_civ2021.dta",clear
rename numind membres__id
merge 1:1 grappe menage vague membres__id using "s04a_me_civ2021.dta",gen(_merge_s04a)

/*      Result                      Number of obs
    -----------------------------------------
    Not matched                           172
        from master                        86  (_merge_s04a==1)
        from using                         86  (_merge_s04a==2)

    Matched                            64,405  (_merge_s04a==3)
    -----------------------------------------


*/

drop if _merge_s04a==2
count
merge 1:1 grappe menage vague membres__id using "s04b_me_civ2021.dta",gen(_merge_s04b)

/*      Result                      Number of obs
    -----------------------------------------
    Not matched                           172
        from master                        86  (_merge_s04b==1)
        from using                         86  (_merge_s04b==2)

    Matched                            64,405  (_merge_s04b==3)
    -----------------------------------------


*/
drop if _merge_s04b==2
count
merge 1:1 grappe menage vague membres__id using "s03_me_civ2021.dta", gen(_merge_s03)

/*      Result                      Number of obs
    -----------------------------------------
    Not matched                           172
        from master                        86  (_merge_s03==1)
        from using                         86  (_merge_s03==2)

    Matched                            64,405  (_merge_s03==3)
    -----------------------------------------


*/
drop if _merge_s03==2
count
keep country year vague hhid grappe menage membres__id zae zaemil departement milieu hhweight resid sexe age alfa scol educ_hi telpor internet s04q06 s04q07 s04q08 s04q09 s04q18a s04q18b s04q28a s04q28b s04q30b s04q29b s04q39 s03q41 s03q42 s03q43 s03q44 s03q45 s03q46 s03q32 _merge_s04a _merge_s04b _merge_s03

rename membres__id numind 
save "CIV_LSMS_2021_Ind.dta", replace 

use "CIV_LSMS_2021_Ind.dta" ,clear
merge m:1 grappe menage vague using "CIV_LSMS_2021_HH.dta"

/*    Result                      Number of obs
    -----------------------------------------
    Not matched                             0
    Matched                            64,491  (_merge==3)
    -----------------------------------------*/

bysort hhid: gen hh_size = _N
save "Ivory Coast 2021 raw.dta", replace 

use "Ivory Coast 2021 raw.dta",clear

drop if age<15
********Country Name****************
gen country_name="Côte d'Ivoire"
********Country Abrevation*********
gen country_abrev="CI" 

*********Country_dataset_year*******
gen country_dataset_year="Côte d'Ivoire LSMS 2021-2022"
...
egen hh_id= concat(grappe menage vague), format(%25.0g) punct(_)
egen ind_id= concat(hhid numind), format(%25.0g) punct(_)

egen sample_strata=group(departement milieu)
gen psu=grappe
*Urban/Rural
...
replace ind_water = 1 if inlist(s11q26b, 1,2,3,4,7,8,9,10,11,14,15,16)
...
foreach var of varlist s14bq02101-s14bq02122 {
    replace shock_any = 1 if `var' == 1
}

* Set to missing if all variables are missing
egen nmiss = rownonmiss(s14bq02101-s14bq02122)
replace shock_any = . if nmiss == 0
drop nmiss
...
save " Ivory_Coast_LSMS_2021.dta",replace 
...
save " Ivory_Coast_LSMS_2021 with missing.dta", replace
...
save " Ivory_Coast_LSMS_2021_Cleaned_Individual_Data_Trimmed.dta", replace

duplicates drop hh_id, force

save " Ivory_Coast_LSMS_2021_Cleaned_Household_Level_Data_Trimmed.dta", replace
