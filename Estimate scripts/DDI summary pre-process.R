...
foreach(dta = dta_list2) %do%
  {
    ...
    dck = dck %>%
      mutate(across(c(female, urban_new, age_group), ~ as.double(.x)))
    if (
      is.na(data_list$Subnational_1_feasible[data_list$File_Name == svy]) &
        "admin1" %in% names(dck)
    ) {
      dck = dck %>% select(-admin1)
    }
    if (
      is.na(data_list$Subnational_2_feasible[data_list$File_Name == svy]) &
        "admin2" %in% names(dck)
    ) {
