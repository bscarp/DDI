...
with_progress({
  ...
  foreach(
  ...
  ) %dofuture%
    {
      ...
      r_adj_name = paste0(
        data_loc2,
        sub("\\.RData", "\\_Adjusted.RData", r_name)
      )
      ...
      dck = dck %>%
        mutate(
          ...
          age_group5 = cut(
            age,
            c(14, 19, 24, 29, 34, 39, 44, 49, 54, 59, 64, 69, 74, 79, Inf),
            c(
              "15 to 19",
              "20 to 24",
              "25 to 29",
          ...
          as_weight = case_when(
            age_sex == "15 to 19 Female" ~ 0.055177596606434,
      ...
      if (grepl("tsu", psu2$`Stata code FINAL November 2024`)) {
        ...
        dck2 = survey::svydesign(
          ...
          survey::postStratify(~age_sex, df_age_sex) %>%
          as_survey()
        dck2$fpc$pps = FALSE
        rm(dck)
        ...
        dck2 = dck %>%
          filter(!is.na(psu) & !is.na(ind_weight) & !is.na(age_sex))
        dck2 = survey::svydesign(
          ids = ~psu,
          weights = ~ind_weight,
          strata = NULL,
      ...
      tab_as_adj = foreach(
        agg_grp = c("All", "female", "age_group"),
        .combine = "full_join",
        .options.future = list(packages = c("srvyr"))
      ) %dofuture%
        {
          ...
          tab = dck2 %>%
            ...
            summarise(
              across(
                c(all_of(dis_a2), all_of(dom_a)),
                list(
                  mean = ~ if_else(
                    sum(!is.na(.x)) < 50,
                    NA,
                    survey_mean(as.numeric(.x) - 1, na.rm = T, df = Inf) * 100
                  ),
                  n = ~ n()
                )
              ),
...
if (
  !file.exists(paste0(
    cen_dir,
    "Downloads/Census/Database/S5_AdjustedPrevalenceRates_Means.xlsx"
  ))
) {
  file.copy(
    paste0(
      cen_dir,
      "Downloads/Census/Database/S5_AdjustedPrevalenceRates_Means - Copy.xlsx"
    ),
    paste0(
      cen_dir,
      "Downloads/Census/Database/S5_AdjustedPrevalenceRates_Means.xlsx"
  ...
  file.copy(
    ...
    paste0(
      cen_dir,
      "Downloads/Census/Database/S6_AdjustedPrevalenceRates_SE.xlsx"
    )
  )
}
mean1 = read_xlsx(
...
merged = foreach(
...
) %dofuture%
  {
    ...
    names(tab_as_adj) = sub(
      "disability_atleast",
      "severe_disability",
      sub(
        "disability_some",
        "moderate_disability",
        sub("disability_any", "disability", names(tab_as_adj))
      )
    )
    names(tab_as_adjb) = sub(
      "disability_atleast",
      "severe_disability",
      sub(
        "disability_some",
        "moderate_disability",
        sub("disability_any", "disability", names(tab_as_adjb))
      )
    )
    names(tab_as_adj) = sub(
      "_mean",
      "_adjusted",
      sub("_any_mean", "_adjusted", names(tab_as_adj))
    )
    names(tab_as_adjb) = sub(
      "_mean_se",
      "_adjusted",
      sub("_orany_mean_se", "_adjusted", names(tab_as_adjb))
    )
    db_mean = tab_as_adj %>%
      pivot_wider(
        names_from = disagg,
        names_glue = "{.value} ({disagg})",
        values_from = c(names(tab_as_adj[-c(1:3)]))
      )
    db_se = tab_as_adjb %>%
      pivot_wider(
        names_from = disagg,
        names_glue = "{.value} ({disagg})",
        values_from = c(names(tab_as_adjb[-c(1:3)]))
      )
...
db_se = db_se %>%
  mutate(across(!survey & !admin & !level, ~ if_else(.x == "Inf", NA, .x))) %>%
  mutate(across(
    !survey & !admin & !level & where(is.character),
    ~ as.numeric(.x)
  ))
