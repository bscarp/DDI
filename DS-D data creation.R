...
data0 = data0 %>% mutate(DifficultyName = str_replace_all(DifficultyName,setNames(unique(df_disability_t$DifficultyName)[c(1,3,4,6:11,2)],unique(data0$DifficultyName)[c(10,11,3,4,5,8,9,7,6,2)])), DifficultyName = ifelse(DifficultyName=="disability","Disability",DifficultyName))
...
data1 = data1 %>% mutate(DifficultyName = str_replace_all(DifficultyName,setNames(unique(df_disability_t$DifficultyName)[c(1,3,4,6:11,2)],unique(data1$DifficultyName)[c(10,11,3,4,5,8,9,7,6,2)])), DifficultyName = ifelse(DifficultyName=="disability","Disability",DifficultyName))
...
rm(ddi_2024)
df_country = df_country %>% select(country, Region) %>% setNames(c("label", "group"))
...
df_group2 = df_group %>% mutate(label = sub("Adults ages 15 to 29", "Adults ages 25 to 29", label), value = sub("Adults ages 15 to 29", "Adults ages 25 to 29", value))
df_disability = tibble(label = c("Disability versus no disability", "Severe versus moderate versus no disability", "Severe versus moderate or no disability",
                  "Disability by type"), value = c(1,2,3,4))
df_disability2 = tibble(label = unique(data1$DifficultyName))
df_disability2 = df_disability2 %>% mutate(value = label)

df_lang = tibble(label = c("English", "French", "Spanish", "Russian", "中文", "Arabic"), value = c("en", "fr", "es", "ru", "zh", "ar"))
...
df_country2 = data1 %>% select(Country,admin) %>% distinct() %>% filter(!admin == "National")
...
save(df_country, df_country2, df_group, df_group2, df_disability, df_disability2, df_lang, key_m, df_country, df_static, file = "DS-E/Data.RData")
