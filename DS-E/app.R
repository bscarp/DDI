...
ui <- page_navbar(
  ...
  # Landing page
  nav_item(a(href="https://ds-e.disabilitydatainitiative.org",  i18n$t("Home"))),

  #Selectors
  sidebar = sidebar(id = "sidebar", title = tagList(shiny.i18n::usei18n(i18n), ""),
                    accordion(open = FALSE,
                              accordion_panel(title = i18n$t("Language"), value =  "Language", virtualSelectInput("selected_language", NULL, choices = prepare_choices(df_lang, label, value), selected = i18n$get_key_translation(), keepAlwaysOpen = TRUE)),
                              conditionalPanel(condition = "input.nav == 'across'", accordion_panel(title = i18n$t("Countries (select multiple)"), value =  "Countries (select multiple)", virtualSelectInput("country", NULL, choices = prepare_choices(df_country, label, value, group), multiple = TRUE, search = TRUE, selected = c("South Africa", "Kenya", "Uganda"), keepAlwaysOpen = TRUE, popupDropboxBreakpoint = "500px"),
                                                                                                    actionLink("selectall", i18n$t("Select all countries")), actionLink("reset", i18n$t("Reset countries"))), ns = NS(NULL)),
                              conditionalPanel(condition = "input.nav == 'within'", accordion_panel(title = i18n$t("Country (select single)"), value =  "Country (select single)", virtualSelectInput("country_sin", NULL, choices = prepare_choices(df_country, label, value, group), search = TRUE, selected = "Guatemala", keepAlwaysOpen = TRUE, popupDropboxBreakpoint = "500px")), ns = NS(NULL)),
                              div(style = "text-align: right;", dropdownButton(i18n$t("The Methods tab above has definitions and details about breakdowns"), status = 'info', icon = icon('info'))),
                              conditionalPanel(condition = "input.nav == 'within' & input.h2 == 't4'", accordion_panel(title = i18n$t("Select the subnational level"), value =  "Select the subnational level", virtualSelectInput("admin", NULL, c("Subnational division 1", "Subnational division 2", "Alternative subnational division"), search = TRUE, selected = "Subnational division 1", keepAlwaysOpen = TRUE, popupDropboxBreakpoint = "500px")), ns = NS(NULL)),
                              conditionalPanel(condition = "input.nav != 'citation'", accordion_panel(title = i18n$t("Indicators"), value =  "Indicators", virtualSelectInput("indicator", NULL, prepare_choices(key_m, IndicatorName, IndicatorName, Group, alias = Original), search = TRUE, selected = "Multidimensional poverty", position = "auto", keepAlwaysOpen = TRUE, popupDropboxBreakpoint = "500px")), ns = NS(NULL)),
                              conditionalPanel(condition = "input.nav != 'citation'", accordion_panel(title = i18n$t("Population Groups"), value =  "Population Groups", virtualSelectInput("group", NULL, choices = prepare_choices(df_group, label, value), selected = "All adults (ages 15 and older)", keepAlwaysOpen = TRUE, popupDropboxBreakpoint = "500px")), ns = NS(NULL)),
                              conditionalPanel(condition = "input.nav == 'across' | (input.nav == 'within' & input.h2 == 't4')", accordion_panel(title = i18n$t("Disability breakdown"), value =  "Disability breakdown", virtualSelectInput("disability", NULL, choices = prepare_choices(df_disability, label, value), selected = 1, keepAlwaysOpen = TRUE, popupDropboxBreakpoint = "500px")), ns = NS(NULL)),
                              conditionalPanel(condition = "input.nav == 'within' & input.h2 == 't3'", accordion_panel(title = i18n$t("Disability group"), value =  "Disability group", virtualSelectInput("disability2", NULL, choices = prepare_choices(df_disability2, label, value), selected = "Disability", keepAlwaysOpen = TRUE, popupDropboxBreakpoint = "500px")), ns = NS(NULL)),
                              ...
                              conditionalPanel(condition = "input.nav == 'within' & input.h2 == 't3'", accordion_panel(title = i18n$t("Indicator scale"), value =  "Indicator scale", noUiSliderInput("scale", NULL, min = 0, max = 100, c(0,100))), ns = NS(NULL)),
                              conditionalPanel(condition = "input.nav == 'across' & input.h1 == 't1'", accordion_panel(title = i18n$t("Pick a colour theme"), value =  "Pick a colour theme", virtualSelectInput("colour1", NULL, choices =c("Viridis" = 1, "Turbo" = 2, "Plasma" = 3), selected = 2, keepAlwaysOpen = TRUE, popupDropboxBreakpoint = "500px")), ns = NS(NULL)),
                              conditionalPanel(condition = paste("input.nav == 'within' & input.h2 == 't3' & [", paste(paste("'", key_m$IndicatorName[!is.na(key_m$Direction)],"'",sep=""),collapse=","),"].includes(input.indicator)",sep=""),
                                               accordion_panel(title = i18n$t("Pick a colour theme"), value =  "Pick a colour theme", virtualSelectInput("colour2", NULL, choices = c("Viridis" = 1, "Mako" = 2, "Rocket" = 3), selected = 3, keepAlwaysOpen = TRUE, popupDropboxBreakpoint = "500px")), ns = NS(NULL)),
                              conditionalPanel(condition = paste("input.nav == 'within' & input.h2 == 't3' & [", paste(paste("'", key_m$IndicatorName[ is.na(key_m$Direction)],"'",sep=""),collapse=","),"].includes(input.indicator)",sep=""),
                                               accordion_panel(title = i18n$t("Pick a colour theme"), value =  "Pick a colour theme", virtualSelectInput("colour3", NULL, choices = c("Viridis" = 1, "Mako" = 2, "Rocket" = 3), selected = 2, keepAlwaysOpen = TRUE, popupDropboxBreakpoint = "500px")), ns = NS(NULL))
                    )
  ),
  nav_panel(i18n$t("Cross-country estimates"), value = "across",
            navset_card_underline(id = "h1",
                                  nav_panel(value = 't1', i18n$t("Graph"), h4(textOutput("title1")), h6(textOutput("key1")), h6(textOutput("ind1")), girafeOutput("stat_top_gra")),
                                  nav_panel(value = 't2', i18n$t("Table"), h4(textOutput("title2")), h6(textOutput("key2")), h6(textOutput("ind2")), DTOutput("stat_top_tab"))
            )),
  nav_panel(i18n$t("Estimates within countries"), value = "within",
            navset_card_underline(id = "h2",
                                  nav_panel(value = 't3', i18n$t("Map"), h4(textOutput("title3")), textOutput("ind3"), girafeOutput("stat_cou_map")),
                                  nav_panel(value = 't4', i18n$t("Table"), h4(textOutput("title4")), textOutput("ind4"), DTOutput("stat_cou_tab"))
            )),
  nav_item(a(href="https://www.disabilitydatainitiative.org/ds-e-methods", i18n$t("Methods"), target="_blank")),
...
server <- function(session, input, output) {
  ...
  observe({
    if(input$selectall == 0) {
      ...
      updateVirtualSelect("country", selected = df_country$value, session = session)
  ...
  observe({
    if(input$reset == 0) {
      ...
      updateVirtualSelect("country", selected = c("South Africa", "Kenya", "Uganda"), session = session)
    }
  })
  ...
  observe({
    temp = input$country
    temp2 = df_country %>% mutate(label = i18n_r()$t(label), group = i18n_r()$t(group))
    updateVirtualSelect("country", choices = prepare_choices(temp2, label, value, group), selected = temp, session = session)
  })
  observe({
    temp = input$country_sin
    temp2 = df_country %>% mutate(label = i18n_r()$t(label), group = i18n_r()$t(group))
    updateVirtualSelect("country_sin", choices = prepare_choices(temp2, label, value, group), selected = temp, session = session)
  })
  
  #Change categories for admin selector
  observe({
    temp = if_else(input$admin %in% df_country2$admin[df_country2$Country %in% input$country_sin], input$admin, i18n_r()$t("Subnational division 1"))
    temp2 = df_country2 %>% filter(Country %in% input$country_sin) %>% select(admin) %>% setNames("value") %>% mutate(label = i18n_r()$t(value))
    updateVirtualSelect("admin", choices = prepare_choices(temp2, label, value), selected = temp, session = session)
  })

  #Change categories for indicator selector
  observe({
    temp = input$indicator
    updateVirtualSelect("indicator", choices = key_m %>% mutate(label = i18n_r()$t(IndicatorName)) %>% prepare_choices(label, IndicatorName, Group, alias = Original), 
                        disabledChoices = df_static %>% filter(Country %in% input$country) %>% summarise(min = min(min), .by = IndicatorName) %>% filter(min == "Inf") %>% pull(IndicatorName), selected = temp, session = session)
  })
  ...
  observe({
    ...
    updateVirtualSelect("group", choices = prepare_choices(temp2, label, value), selected = temp, session = session)
  })
  ...
  observe({
    ...
    temp2 = temp2 %>% mutate(label = i18n_r()$t(label))
    updateVirtualSelect("disability", choices = prepare_choices(temp2, label, value), selected = temp, session = session)
  })
  ...
  observe({
    ...
    temp2 = temp2 %>% mutate(label = i18n_r()$t(label))
    updateVirtualSelect("disability2", choices = prepare_choices(temp2, label, value), selected = temp, session = session)  })
  ...
  observe({
    ...
    temp2 = temp2 %>% mutate(label = i18n_r()$t(label))
    updateVirtualSelect("disability3", choices = prepare_choices(temp2, label, value), selected = temp, session = session)  })
  ...
  output$title1 <- renderText({
    paste(i18n_r()$t("Graph showing"), i18n_r()$t(input$indicator), i18n_r()$t("for"), ifelse(length(input$country)==1,i18n_r()$t(input$country),paste(length(i18n_r()$t(input$country)), i18n_r()$t("countries"))), i18n_r()$t("by"), paste0(i18n_r()$t(dis_grp()), collapse = ", "))
  })

  output$title2 <- renderText({
    paste(i18n_r()$t("Table showing"), i18n_r()$t(input$indicator), i18n_r()$t("for"), ifelse(length(input$country)==1,i18n_r()$t(input$country),paste(length(i18n_r()$t(input$country)), i18n_r()$t("countries"))), i18n_r()$t("by"), paste0(i18n_r()$t(dis_grp()), collapse = ", "))
  })

  output$title3 <- renderText({
    paste(i18n_r()$t("Map showing"), i18n_r()$t(input$indicator), i18n_r()$t("for"), i18n_r()$t(input$country_sin), i18n_r()$t("for"), i18n_r()$t(input$disability2))
  })

  output$title4 <- renderText({
    paste(i18n_r()$t("Table showing"), i18n_r()$t(input$indicator), i18n_r()$t("for"), i18n_r()$t(input$country_sin), i18n_r()$t("at"), i18n_r()$t(input$admin), i18n_r()$t("by"), paste0(i18n_r()$t(dis_grp()), collapse = ", "))
  })

  output$key1 <- output$key2 <- renderText({
    key_m %>% filter(IndicatorName == input$indicator) %>% pull(`Key message`) %>% i18n_r()$t() %>% paste(i18n_r()$t("Key message:"), .)
  })

  output$ind1 <- output$ind2 <- output$ind3 <- output$ind4 <- renderText({
    key_m %>% filter(IndicatorName == input$indicator) %>% pull(Tooltip) %>% i18n_r()$t() %>% paste(i18n_r()$t("Indicator definition:"), .)
  })

  output$stat_top_gra <- renderGirafe({
    # draw the plot using data
    data_g = data_sel0() %>% mutate(Country = i18n_r()$t(Country), DifficultyName = i18n_r()$t(DifficultyName), label = paste0(Country,"\n",DifficultyName,"\n",if_else(is.na(Value), i18n_r()$t("Insufficient Sample Size"), paste0(round(Value,1),"%"))))
    plot = ggplot(data = data_g) + geom_col_interactive(mapping = aes(y = Value, x = Country, fill = DifficultyName, tooltip = label, data_id = Country), position = "dodge") +
      scale_y_continuous(name = NULL, labels = scales::label_percent(scale = 1), limits = c(0,100)) + labs(caption = ifelse(length(input$country)>3,i18n_r()$t("Data source(s): See table for source list"),source_all())) +
  ...
  output$stat_top_tab <- renderDT({
    ...
    data_sel0() %>% mutate(Value = Value/100) %>% pivot_wider(names_from = c(IndicatorName,DifficultyName,PopulationName),names_glue = "{DifficultyName}",values_from = Value) %>% select(-c(admin,level)) %>%
      datatable(., colnames = i18n_r()$t(names(.)), caption = htmltools::tags$caption(style = "caption-side: bottom; text-align: left;",HTML(paste0(source_all(), "<br/>", i18n_r()$t("A blank cell indicates that the estimate is not available."))))) %>%
  ...
  output$stat_cou_tab <- renderDT({
    ...
    data_t %>% mutate(Value = Value/100) %>% pivot_wider(names_from = c(IndicatorName,DifficultyName,PopulationName),names_glue = "{DifficultyName}",values_from = Value) %>%
      datatable(., colnames = i18n_r()$t(sub("level", "Level",sub("ISOCode", "ISO Code", names(.)))), caption = htmltools::tags$caption(style = "caption-side: bottom; text-align: left;", HTML(paste0(source_sin(), "<br/>", i18n_r()$t("A blank cell indicates that the estimate is not available."))))) %>%
      formatPercentage(columns = dis_grp(), digits = 1)
  })
  ...
  source_all = reactive({
    df_static %>% filter(Country %in% input$country, IndicatorName == input$indicator) %>% pull(source) %>% sub("_"," ",.) %>% sub("_"," ",.) %>% sub("_","-",.) %>% i18n_r()$t() %>% paste0(collapse = ", ") %>% paste(i18n_r()$t("Data source(s):"), .)
  })

  source_sin = reactive({
    df_static %>% filter(Country == input$country_sin, IndicatorName == input$indicator) %>% pull(source) %>% sub("_"," ",.) %>% sub("_"," ",.) %>% sub("_","-",.) %>% i18n_r()$t() %>% paste(i18n_r()$t("Data source:"), .)
  })
}
