...
ui <- page_navbar(
  ...
  # Landing page
  nav_panel(id = "home", title = "Home",
            useShinyjs(),
            extendShinyjs(text = 'shinyjs.hideSidebar = function(params) { $("body").addClass("sidebar-collapse") }', functions = c("hideSidebar")),
  ...
  #Selectors
  sidebar = sidebar(
               conditionalPanel(condition = "input.tabs == 't1'", selectInput("country", "Countries (select mutiple)", df_country, multiple = TRUE, selected = "Namibia"), ns = NS(NULL)),
               conditionalPanel(condition = "input.tabs == 't2'|input.tabs == 't3'", selectInput("country_sin", "Country (select single)", df_country, selected = "Namibia"), ns = NS(NULL)),
               selectInput("indicator", "Indicators", df_indicator,selected = "Multidimensional poverty"),
               selectInput("group", "Population Groups", df_group,selected = "All adults (ages 15 and older)"),
               conditionalPanel(condition = "input.tabs == 't1'|input.tabs == 't3'", selectInput("disability", "Disability groups", df_disability,selected = 1), ns = NS(NULL)),
               conditionalPanel(condition = "input.tabs == 't2'", selectInput("disability2", "Disability groups", df_disability2,selected = "Disability"), ns = NS(NULL))
  ),
  nav_panel("Cross-country estimates",
            navset_card_underline(id = "tabs",
                                  nav_panel(id = 't1', value = 't1', "Graph", girafeOutput("stat_top_gra")),
                                  nav_panel(id = 't1', value = 't1', "Table", DTOutput("stat_top_tab"))
            )),
  nav_panel("Estimates within countries",
            navset_card_underline(id = "tabs",
                                  nav_panel(id = 't2', value = 't2', "Map", girafeOutput("stat_cou_map")),
                                  nav_panel(id = 't3', value = 't3', "Table", DTOutput("stat_cou_tab"))
            )),
)
...
server <- function(session, input, output) {
  ...
  dis_grp = reactive({
    unlist(case_when(input$disability == 1 ~ list(c("No disability","Disability")),
                     ...
                     input$disability == 4 ~ list(c("No disability","Seeing difficulties","Hearing difficulties","Mobility difficulties", "Cognition difficulties",
