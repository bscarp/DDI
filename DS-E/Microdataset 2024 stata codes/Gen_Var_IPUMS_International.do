...
use "D:\ipumsi_00048.dta\ipumsi_00048.dta", clear
...
********Country Abrevation*********
gen country_abrev="MUS" if country==480
...
replace country_abrev="KHM" if country==116

*********Country_dataset_year*******
gen country_dataset_year=48001 if country==480
...
replace country_dataset_year=70401 if country==704
label define country_dataset_year 10401 "Myanmar IPUMS 2014" 48001 "Mauritius IPUMS 2011" 50401 "Morocco IPUMS 2014" 68601 "Senegal IPUMS 2013" 70401 "Vietnam IPUMS 2009" 71001 "South Africa IPUMS 2011" 71002 "South Africa IPUMS 2016" 74001 "Suriname IPUMS 2012" 80001 "Uganda IPUMS 2014" 83401 "Tanzania IPUMS 2012" 85801 "Uruguay IPUMS 2011"
...
replace ind_electric=1 if country==704 &  vn2009a_light==1
replace ind_electric=0 if country==704 &  inrange(vn2009a_light, 2, 5)
...
replace ind_motorcycle=1 if vn2009a_moto==1 & country==704
replace ind_motorcycle=0 if vn2009a_moto==2 & country==704
...
replace ind_water = 1 if country == 704 & inlist( vn2009a_watsrc , 1, 2, 4, 6, 8)
replace ind_water = 0 if country == 704 & inlist( vn2009a_watsrc , 3, 5, 7)
...
replace toilet_use= 1 if country== 704 & inlist(vn2009a_toilet, 2, 1)
replace toilet_use= 0 if country== 704 & inlist(vn2009a_toilet, 3, 4)

clonevar ind_toilet=toilet_use if country == 504 | country == 104 | country == 686 | country == 834 | country == 858 | country == 740 | country == 704
...
replace ind_floor=. if floor==0
replace ind_floor=. if inlist(country, 704, 740, 480) /*floor variable not avaiable for south africa 2011, Vietnam, Mauritius*/
replace ind_floor=. if sample==710201101
*Roof 

gen ind_roof= inlist(roof, 10, 11, 12, 14, 16, 19, 31, 34, 36, 38, 26)
replace ind_roof=. if roof==0 | roof==99
replace ind_roof=. if country==480 /*roof variable not avaiable for Mauritius*/

*Wall

gen ind_wall= inlist(wall, 501, 502, 505, 507, 510, 512, 516, 518, 519, 520, 521, 522, 523)
replace ind_wall=. if wall==0 | wall==999
replace ind_wall=. if country==480 /*wall variable not avaiable for Mauritius*/
...
global country_list MAR MMR MUS SEN SUR TZA UGA URY VNM ZAF ZAF1
