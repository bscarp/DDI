...
Website:
*******************************************************************************/
use "C:\Users\kavip\Downloads\ipumsi_00050.dta\ipumsi_00050.dta" ,clear
...
* 8,236,773  *(2,104,152 observations deleted)

gen country_name="Vietnam" 

gen country_abrev="VNM" 

gen country_dataset_year="Vietnam IPUMS 2019" if country==704

rename geo1_vn2019 admin1
rename geo2_vn2019 admin2
rename regnvn admin_alt 

gen hh_weight= hhwt
gen ind_weight=perwt
gen sample_strata=vn2019a_strata
...
replace age_grp=6 if age>=40 & age<=44
replace age_grp=7 if age>=45 & age<=49
replace age_grp=8 if age>=50 & age<=54
replace age_grp=9 if age>=55 & age<=59
replace age_grp=10 if age>=60*/
...
gen work_informal=cond(mi(classwkd),.,cond(inlist(classwkd, 120, 310),1,0))
...
*motor
gen ind_motorcycle= (vn2019a_motorcyc==1)
replace ind_motorcycle=. if vn2019a_motorcyc==9

*Bike
gen ind_bike= ( vn2019a_bike ==1)
replace ind_bike=. if vn2019a_bike==9
...
gen ind_water=inlist( vn2019a_watsrc, 1,2,3,4,6,8)

*Electricity

gen ind_electric=(vn2019a_light==1)

*Sanitation

gen ind_toilet=inlist( vn2019a_toilet, 1,2)

gen ind_wall=(vn2019a_walls==1)
replace ind_wall=. if inlist(vn2019a_walls, 8,9)
gen ind_roof=( vn2019a_roof ==1)
replace ind_roof=. if inlist(vn2019a_roof, 8,9)

gen ind_livingcond=(ind_roof==1&ind_wall==1)
replace ind_livingcond=. if ind_wall==. & ind_roof==.

rename computer computer1
...
save "D:\DDI\Vietnam_IPUMS_2019.dta", replace
...
save "D:\DDI\Vietnam_IPUMS_2019 with missing.dta", replace
...
save "D:\DDI\Vietnam_IPUMS_2019_Cleaned_Individual_Data_Trimmed.dta", replace

duplicates drop hh_id, force

save "D:\DDI\Vietnam_IPUMS_2019_Cleaned_Household_Level_Data_Trimmed.dta", replace
