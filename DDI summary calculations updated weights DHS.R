...
foreach(r_name = dhs_list, .options.future = list(packages = c("tidyverse","haven"))) %dofuture% {
  ...
  tab_m_nr1 = foreach(agg_grp=c("All","female","urban_new","age_group"), .combine = "full_join") %:% foreach(dis_grp=c("disability_any","disability_sev","disability_atleast"), .combine = "full_join", .options.future = list(packages = c("tidyverse","haven"))) %dofuture% {
    ...
    tab = bind_rows(dck2a %>% group_by({{agg}}, {{dis}}, admin1) %>% summarise(across(any_of(ind_a), list(mean = ~if_else(sum(!is.na(.x))<50,NA,survey_mean(.x,na.rm = T)*100))),.groups = "drop") %>% complete({{agg}}, {{dis}}, admin1),
                    dck2a %>% mutate(admin1 = factor(admin1_n+1,labels = "National")) %>% group_by({{agg}}, {{dis}}, admin1) %>% summarise(across(any_of(ind_a), list(mean = ~if_else(sum(!is.na(.x))<50,NA,survey_mean(.x,na.rm = T)*100))),.groups = "drop") %>% complete({{agg}}, {{dis}}, admin1)) %>%
  ...
  if(sum(!is.na(dck2c$health_exp_hh))>0) {
    tab_m_nr3 = foreach(agg_grp=c("All","female","urban_new","age_group"), .combine = "full_join") %:% foreach(dis_grp=c("disability_any","disability_sev","disability_atleast"), .combine = "full_join", .options.future = list(packages = c("tidyverse","haven"))) %dofuture% {
      ...
      dis = as.symbol(dis_grp)
      agg = ifelse(agg_grp=="All",agg_grp,as.symbol(agg_grp))
      tab = bind_rows(dck2c %>% group_by({{agg}}, {{dis}}, admin1) %>% summarise(across(any_of(ind_a), list(mean = ~if_else(sum(!is.na(.x))<50,NA,survey_mean(.x,na.rm = T)*100))),.groups = "drop") %>% complete({{agg}}, {{dis}}, admin1),
                      dck2c %>% mutate(admin1 = factor(admin1_n+1,labels = "National")) %>% group_by({{agg}}, {{dis}}, admin1) %>% summarise(across(any_of(ind_a), list(mean = ~if_else(sum(!is.na(.x))<50,NA,survey_mean(.x,na.rm = T)*100))),.groups = "drop") %>% complete({{agg}}, {{dis}}, admin1)) %>%
        arrange({{agg}}, {{dis}}, admin1) %>% pivot_wider(names_from = {{dis}},values_from = -c(1:3)) %>% mutate(Agg = paste0(agg_grp," = ",{{agg}}),.before = "admin1") %>% select(-1)
    }
  } else {
    tab_m_nr3 = tab_m_nr1 %>% select(Agg,admin1,contains("everattended_new"))
  ...
  if(sum(!is.na(dck2d$anyviolence_byh_12m))>0) {
    tab_m_nr4 = foreach(agg_grp=c("All","female","urban_new","age_group"), .combine = "full_join") %:% foreach(dis_grp=c("disability_any","disability_sev","disability_atleast"), .combine = "full_join", .options.future = list(packages = c("tidyverse","haven"))) %dofuture% {
      ...
      dis = as.symbol(dis_grp)
      agg = ifelse(agg_grp=="All",agg_grp,as.symbol(agg_grp))
      tab = bind_rows(dck2d %>% group_by({{agg}}, {{dis}}, admin1) %>% summarise(across(any_of(ind_a), list(mean = ~if_else(sum(!is.na(.x))<50,NA,survey_mean(.x,na.rm = T)*100))),.groups = "drop") %>% complete({{agg}}, {{dis}}, admin1),
                      dck2d %>% mutate(admin1 = factor(admin1_n+1,labels = "National")) %>% group_by({{agg}}, {{dis}}, admin1) %>% summarise(across(any_of(ind_a), list(mean = ~if_else(sum(!is.na(.x))<50,NA,survey_mean(.x,na.rm = T)*100))),.groups = "drop") %>% complete({{agg}}, {{dis}}, admin1)) %>%
        arrange({{agg}}, {{dis}}, admin1) %>% pivot_wider(names_from = {{dis}},values_from = -c(1:3)) %>% mutate(Agg = paste0(agg_grp," = ",{{agg}}),.before = "admin1") %>% select(-1)
    }
  } else {
    tab_m_nr4 = tab_m_nr1 %>% select(Agg,admin1,contains("everattended_new"))
  ...
  if(sum(!is.na(dck2c$health_exp_hh))>0) {
    tab_P4_nr3 = foreach(dom_grp=dom_a, .options.future = list(packages = c("tidyverse","haven")),.combine = "rbind") %dofuture% {
      ...
      options(survey.lonely.psu = "adjust")
      dom = as.symbol(dom_grp)
      tab = bind_rows(dck2c %>% mutate(disability_any = as.numeric(disability_any)-1) %>% group_by(admin1) %>% filter({{dom}}==1, .preserve = TRUE) %>% summarise(across(any_of(ind_a), list(mean = ~if_else(sum(!is.na(.x))<50,NA,survey_mean(.x,na.rm = T)*100))),.groups = "drop") %>% complete(admin1),
                      dck2c %>% mutate(disability_any = as.numeric(disability_any)-1,admin1 = factor(admin1_n+1,labels = "National")) %>% group_by(admin1) %>% filter({{dom}}==1, .preserve = TRUE) %>% summarise(across(any_of(ind_a), list(mean = ~if_else(sum(!is.na(.x))<50,NA,survey_mean(.x,na.rm = T)*100))),.groups = "drop") %>% complete(admin1)) %>%
        arrange(admin1) %>% mutate(domain = dom_grp,.before = "admin1")
    }
  } else {
    tab_P4_nr3 = tab_P4_nr1 %>% select(domain,admin1,contains("everattended_new"))
  ...
  if(sum(!is.na(dck2d$anyviolence_byh_12m))>0) {
    tab_P4_nr4 = foreach(dom_grp=dom_a, .options.future = list(packages = c("tidyverse","haven")),.combine = "rbind") %dofuture% {
      ...
      options(survey.lonely.psu = "adjust")
      dom = as.symbol(dom_grp)
      tab = bind_rows(dck2d %>% mutate(disability_any = as.numeric(disability_any)-1) %>% group_by(admin1) %>% filter({{dom}}==1, .preserve = TRUE) %>% summarise(across(any_of(ind_a), list(mean = ~if_else(sum(!is.na(.x))<50,NA,survey_mean(.x,na.rm = T)*100))),.groups = "drop") %>% complete(admin1),
                      dck2d %>% mutate(disability_any = as.numeric(disability_any)-1,admin1 = factor(admin1_n+1,labels = "National")) %>% group_by(admin1) %>% filter({{dom}}==1, .preserve = TRUE) %>% summarise(across(any_of(ind_a), list(mean = ~if_else(sum(!is.na(.x))<50,NA,survey_mean(.x,na.rm = T)*100))),.groups = "drop") %>% complete(admin1)) %>%
        arrange(admin1) %>% mutate(domain = dom_grp,.before = "admin1")
    }
  } else {
    tab_P4_nr4 = tab_P4_nr1 %>% select(domain,admin1,contains("everattended_new"))
